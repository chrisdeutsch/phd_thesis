...
and the loss function in \Cref{eq:loss_gradboost} to fit a binary classifier to
...
thesis. The description is based on Ref.~\cite{Friedman:2001wbq} with some
modifications (e.g.\ allowing training data to be weighted).
...
\item For $m = 1$ to $M$:
  ...
  \item The leaf node constants of the regression tree,
    ...
    Newton's method.\footnote{The starting point for Newton's method is taken to
      be $c_{jm} = 0$ for all leaf nodes.} This yields updated leaf node
    constants
    \begin{align*}
...
\item The final prediction of the boosting procedure, $F_{M\hspace{-0.07em}}(x)$, can be used
  to estimate the probability $\mathbb{P}(Y = +1 \mid \myvec{X} = \myvec{x})$
  according to \Cref{eq:gradboost_probas} as
  \begin{align*}
