...
Methods of multivariate analysis (MVA) have been used in HEP for decades to
maximise the insights gained from experimental data. In the past, these analyses
were usually performed by hand, which is a difficult and time-consuming process,
in particular when working with high-dimensional data. Over the past decade,
...
is provided by algorithms that construct predictive models using samples of
training data, which is a process referred to as \emph{machine learning}.

Arguably the most common type of machine learning algorithms used in HEP fall
under the category of \emph{supervised learning}. The goal of supervised
learning is the construction of models that predict an outcome~$\myvec{Y}$ using
a set of predictors~$\myvec{X}$ by learning from a sample of labelled training
data~$\{ (\myvec{x}_i, \myvec{y}_i) \}_{i=1}^{N}$. Depending on the type of
outcome one distinguishes between classification (categorical outcomes) and
regression (e.g.~continuous, count, or ordinal outcomes). Supervised learning is
particularly applicable in HEP due to the ability to generate large amounts of
labelled training data using Monte Carlo simulation, the simulations being
heavily tuned to experimental observations to accurately represent the
data-generating processes. Nowadays, machine learning algorithms represent the
state of the art for many tasks in HEP such as particle identification.
...
in~\Cref{sec:neural_networks}, including a discussion of a type of network
...
composed of an ensemble of \emph{decision trees}. This ensemble is created using
an algorithm referred to as \emph{boosting}, which iteratively fits shallow
...
to emphasise prediction errors made by previous iterations of the
algorithm. Finally, the predictions of the ensemble of trees are combined with
...
discussed hereafter. First, constraints are set on the structure of the tree as
a means to limit its complexity. For example, the maximum depth of the tree or
the maximum number of leaf nodes in the tree could be limited. The following
steps are assumed to be subject to the chosen constraints. In the ideal case,
regression trees are constructed by finding a partitioning~$\{R_j\}_{j=1}^{J}$
and leaf node constants~$\{c_j\}_{j=1}^{J}$ such that the mean squared error
(MSE) given by
\begin{align*}
  \text{MSE} = \frac{\sum_{i = 1}^{N} w_i \left( y_i - h\bigl( \myvec{x}_i; \{c_j, R_j\}_{j=1}^{J} \bigr) \right)^2}{\sum_{i = 1}^{N} w_i}
\end{align*}
is minimised, where~$\{\myvec{x}_i, y_i, w_i\}_{i=1}^{N}$ is a sample of
training data with scalar targets~$y_i$ and sample weights~$w_i$. This
optimisation problem is not easily solved for trees with depths larger than one.
Instead, one resorts to using a greedy strategy. At any stage of the tree
growing algorithm, the split leading to the largest reduction in MSE is
chosen. Concretely, consider a candidate split of a parent node (P) into a left
(L) and right (R) daughter node. Let~$(y, w)$ be the tuple of regression target
and weight for a given training example. Moreover, let $T_{\text{P}}$,
$T_{\text{L}}$, and $T_{\text{R}}$ denote the set of $(y, w)$ for training
examples populating the parent node, left daughter node, and right daughter
node, respectively. The reduction in MSE by the candidate split is given by
\begin{align*}
  \Delta\text{MSE} =
