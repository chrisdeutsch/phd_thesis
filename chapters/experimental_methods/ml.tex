...
making up an NN are referred to as \emph{layers}. An illustrative example of a
simple NN is the \emph{multilayer perceptron} (MLP). The basic element of
MLPs--and many other types of NNs--are \emph{densely connected layers} that
...
weight matrix and bias vector, respectively, and $\phi_i$ is an activation
function that is applied element-wise. An MLP with $N$ hidden layers is given by
the composition of $N + 1$ functions of the form displayed in
...
example (e.g.\ networks with multiple inputs or outputs). Therefore, it is often
useful to express the order of computation in an NN as a directed graph with
nodes representing individual layers. For the MLP example, this graph is given
by
\begin{align*}
  (\text{Input} \to) \, \text{Dense}_{1} \to \dots \to \text{Dense}_{N + 1} \, (\to \text{Output}) \,\text{,}
\end{align*}
where the layers denoted as ``Input'' and ``Output'' do not perform any
computation a
...
refers to the input to the MLP, $\text{Dense}_{i}$ represents
the $i$-th densely connected layer, and ...
...
\footnote{This is equivalent to the loss function used for gradient boosting in
  \Cref{eq:loss_gradboost} noting that the class labels are now $Y \in \{0, 1\}$
  and th.}
