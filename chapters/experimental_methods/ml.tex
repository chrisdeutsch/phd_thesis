...
\begin{figure}[htbp]
  ...
  \caption{Computational graph of a single LSTM cell for time step
    $t$. Operations in circles in circles represent element-wise multiplication
    ($\times$), addition ($+$), and element-wise application of the $\tanh$
    activation function ($\myvec{\phi}$).}
  \label{fig:lstm}
\end{figure}
