...
of the algorithm estimate the negative gradient of the loss function with
respect to~$F(\myvec{x})$, which is evaluated at the function estimate of the
previous boosting iteration. In gradient descent algorithms, the step size of
the update is sometimes determined by performing a line search along the
direction of steepest descent. For the \textsc{TreeBoost} algorithm, the step
size is determined in step 2c) on a per-leaf basis.
