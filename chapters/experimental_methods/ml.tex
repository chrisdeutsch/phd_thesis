...
performed using decision trees. Given a sample of training data, the additive
model is fit using an iterative procedure in which the $m$-th stage of the
...
and $\gamma_{m}$ while leaving all other parameters fixed~\cite{hastie09}. The
following is concerned with a boosting algorithm referred to as \emph{gradient
...
function. As a result, gradient boosting is a useful tool to solve numerous
prediction tasks.
...
\end{align}
be a loss function and $F$~a function of the predictors. The function $F^{*\!}$
...
\begin{align*}
  &\mathbb{P}(Y = +1 \mid \myvec{X} = \myvec{x}) = \frac{1}{1 + \exp\bigl( -F^{*\!}(\myvec{x}) \bigr)}
  &&\mathbb{P}(Y = -1 \mid \myvec{X} = \myvec{x}) = \frac{1}{1 + \exp\bigl( +F^{*\!}(\myvec{x}) \bigr)} \,\text{.}
\end{align*}
...
$F^*$ using a finite sample of training data.
