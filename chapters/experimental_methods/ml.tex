...
Neural networks (NNs) are parametric functions...



For example, a \emph{multilayer perceptron} is a NN composed of transformations
of the form
\begin{align*}
  f_i(\myvec{x}; \myvec{W}_i, \myvec{b}_i) = \phi_i(\myvec{W}_i \myvec{x} + \myvec{b}_i) \,\text{,}
\end{align*}
where $\myvec{W}_i$ and $\myvec{b}_i$ is a weight matrix and bias vector,
respectively, and $\phi_i$ is an element-wise activation function. These
transformations are performed sequentially such that the prediction of a
multilayer perceptron consisting of $N$ layers is given by
\begin{align*}
  f\bigl( \myvec{x}; \{\myvec{W}_i, \myvec{b}_i\}_{i=1}^{N} \bigr) =
  \bigl(f_N \circ f_{N - 1} \circ \dots \circ f_1\bigr)\bigl(\myvec{x}; \{\myvec{W}_i, \myvec{b}_i\}_{i=1}^{N} \bigr) \,\text{.}
\end{align*}
This approach generalises to other types of transformations.
...
are a large class of parametric functions


that can be used to approximate any function~\cite{}.
...
$\myvec{x}_{\text{out}} = f_i(\myvec{x}_{\text{in}}; \myvec{\theta}_i)$


An example of such a transformation is a densely connected layer which applies a
transformation of the form
\begin{align*}
  \myvec{x}_{\text{out}} =
\end{align*}
 A
neural network is constructed by composing multiple layers, i.e.\
\begin{align*}
