...
outcome variable one distinguishes between classification (categorical outcomes)
and regression (e.g.~continuous, count, or ordinal outcomes)
algorithms. Supervised learning is particularly applicable in HEP due to the
ability to generate large amounts of labelled training data using Monte Carlo
simulation tools, the simulations being heavily tuned to experimental data and
thus closely reflecting the data-generating processes. Nowadays, machine
learning algorithms represent the state of the art for many tasks in HEP such as
...
% background processes based on reconstructed, event-level observables.
