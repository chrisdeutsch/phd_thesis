...
superior predictive performance compared to a single decision tree. The
following sections discuss the application of decision trees to regression tasks
(regression trees), as well as a \emph{gradient boosting} algorithm that
constructs ensembles of regression trees to solve various prediction tasks.
...
are set on the tree structure as a means to limit the tree complexity, for
example by setting a maximum tree depth.
