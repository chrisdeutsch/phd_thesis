...
sequence occurs. These capabilities are provided by recurrent layers, which are
described in the remainder of this section. The focus of the description lies on
the \emph{many-to-one} and \emph{many-to-many} ($N = M$) cases.
...
network. In the following, these concepts are illustrated using the example of
\emph{long short-term memory} (LSTM)
layers~\cite{lstm,gers2000learning}. Consider an input and output sequence of
the same length denoted by $(\myvec{x}_{t})_{t = 1}^{N}$ and
