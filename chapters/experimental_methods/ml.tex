...
network architectures. Among these architectures are \emph{recurrent neural
  networks} (RNN), which allow to operate on ordered, variable-length
...
might be beneficial to include information of the first $t - 1$ elements of a
sequence when processing the $t$-th element. These kinds of computations are the
primary motivation for RNN. This thesis is mostly concerned with the
many-to-one, and many-to-many case for $N = M$.
