...
% \footnote{Comparing this with the approach taken for gradient boosted trees for
%   binary classification

%   Comparing the loss functions for binary classification with gradient
%   boosted trees in  and the binary cross-entropy loss in
%   \Cref{eq:loss_bce},

%    and
%   , the only differences are: the change in class labels,
%   i.e.\ $Y \in \{-1, +1\}$ for gradient boosting ; and

%   However, the ensemble of trees approximates the conditional log-odds for the
%   positive class while the NN approximates the conditional probability of
%   being from t }
...
\end{align*}
Training of NNs using SGD proceeds by first randomly initialising the parameters
of the network ($\myvec{\theta}_{0}$). Afterwards, a number SGD iterations are
performed that consist of drawing a mini-batch from the training data followed
by a parameter update according to
\begin{align*}
  \myvec{\theta}_{t + 1} = \myvec{\theta}_{t} - \eta_{t} \myvec{g}(\myvec{\theta}_{t}) \,\text{,}
\end{align*}
where $\eta_{t}$ is referred to as the learning rate. The learning rate might be
constant with respect to $t$ or follow a pre-defined schedule, for example
exponentially decaying learning rates are frequently used. A modification of SGD
is \emph{SGD with momentum} which improves the convergence in
\begin{align*}
...
force proportional to~$-\myvec{g}(\myvec{\theta})$; a friction-like force
proportional and opposed to the
velocity~$\Delta\myvec{\theta}_{t}$~\cite{Goodfellow-et-al-2016}.

% Exponential decay $\eta_{t} = \eta_{0} (1 - \lambda)^{-t}$
