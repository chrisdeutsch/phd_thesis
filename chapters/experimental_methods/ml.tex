...
update by performing a line search along the direction of steepest descent. For
...
of multiple, usually non-linear, parametric functions. NNs represent a rich
...
activation function that is applied element-wise.\footnote{The element-wise
  ...
  is defined by $\bigl( f(x_1), \dots, f(x_N) \bigr)$.} An MLP with $N$~hidden
layers is given by the composition of $N + 1$ functions of the form displayed in
\Cref{eq:dense_layer}, allowing the MLP prediction to be expressed as
\begin{align*}
...
example. Therefore, the architecture of an NN is frequently expressed as a
...
Consider a binary classification problem with predictors~$\myvec{X}$ and class
labels~$Y$, which take the values of $Y = 1$ and $Y = 0$ for the positive and
negative class, respectively. Moreover, let
$f(\myvec{x}; \myvec{\theta}) \in (0, 1)$ denote the prediction of an NN with
inputs~$\myvec{x}$ and network parameters~$\myvec{\theta}$. A suitable loss
