...
\end{align*}
Often, the model is restricted by fixing the POI~$\mu$ to a constant value. For
example, the model with the restriction $\mu = 0$ is typically referred to as
the \emph{background-only model}. In the restricted case, the best-fit
parameters are determined by the \emph{conditional fit} according to
\begin{align*}
  \hat{\hat{\theta}}(\mu) = \argmax_{\myvec{\theta}} L(\mu, \myvec{\theta}) \,\text{,}
\end{align*}
...
- In general, we have composite hypothesis. Let $\Theta$ denote the full
parameter space of the model.


 The null hypothesis is
\begin{align*}
  H_0: \theta \in \Theta_0 = \{ (\mu, \myvec{\theta}) \,\vert\, (\mu, \myvec{\theta}) \in \Theta \text{ and } \mu = \mu^\prime \} \phantom{\,\text{.}}
\end{align*}
and the alternative hypothesis
\begin{align*}
  H_1: \theta \in \Theta_1 = \{ (\mu, \myvec{\theta}) \,\vert\, (\mu, \myvec{\theta}) \in \Theta \text{ and } \mu \neq \mu^\prime \} \,\text{.}
\end{align*}
These hypotheses are referred to as nested ($\Theta = \Theta_0 \cup \Theta_1$)
and composite
