...
In searches for new physics, an approach referred to as the
...
and low experimental sensitivity, respectively. A problem arises when
considering an experiment that has hardly any sensitivity in distinguishing
between the $\mu = \mu^*$ and $\mu = 0$ hypotheses. In this case, the
statistical power of the test approaches $\alpha$, that is,
$1 - \beta \to \alpha$, which is illustrated in~\Cref{fig:cls_qmu_lowpower}. In
practice, a signal to which the experiment has little sensitivity will still be
excluded with a probability of at least~$\alpha$ under the background-only
hypothesis. In HEP these \emph{spurious exclusions} are undesired, motivating
the introduction of an alternative test statistic that takes into account the
...
The \CLs technique addresses the spurious exclusion of signals by introducing a
modified test statistic~\cite{Cowan:2010js}
\begin{align}
  \CLs = \frac
  ...
  \label{eq:cls}
\end{align}
where $\tilde{q}_{\mu, \text{obs}}$ is the observed value of the test
statistic. The \CLs statistic modifies $p_\mu$ (the numerator) by dividing it by
the probability of observing a value of \qmutilde larger than the observed value
under the background-only hypothesis.

% (observed power of the test)
