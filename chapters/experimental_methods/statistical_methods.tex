...
are nuisance parameters (NPs) constrained by auxiliary measurements, which will
be introduced shortly. The relevance of the four factors $\gamma_{csb}$,
...
\item $\eta_{cs}(\myvec{\alpha})$ represents a normalisation factor applied
  ...
  freely since it is a function of the NPs~$\myvec{\alpha}$, which are
  themselves constrained by auxiliary measurements. This factor is included to
  account for uncertainties on the normalisation of process~$s$ in channel~$c$.

\item $\phi_{cs}$ represents a \emph{free} normalisation factor applied
  uniformly to all bins of the parameterised histogram for a given process~$s$
  in channel~$c$. These parameters are optional, i.e.\ they can be fixed to
  unity for a given process/channel depending on the statistical model one wants
  to construct.
...
\begin{align*}
  ...
  \,\Biggr]
  \times L_{\text{aux}}(\myvec{\alpha}, \myvec{\gamma}) \,\text{,}
\end{align*}
where $L_{\text{aux}}(\myvec{\alpha}, \myvec{\gamma})$ refers to the likelihood
function of the auxiliary measurements that constrain the NPs~$\myvec{\alpha}$
and $\myvec{\gamma}$.
...
\begin{align*}
  L_{\text{aux}}(\myvec{\alpha}, \myvec{\gamma}) =
  \Biggl[\, \prod_{p \in A} f_p(a_p \vert \alpha_p)     \,\Biggr]
  \Biggl[\, \prod_{c \in \mathcal{C}} \prod_{b \in \mathcal{B}_{c}} \pois(m_{cb} \vert \gamma_{cb} \tau_{cb}) \,\Biggr]
\end{align*}
