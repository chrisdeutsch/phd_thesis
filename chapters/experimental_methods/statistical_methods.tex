...
Techniques of statistical inference commonly used at the ATLAS experiment to
interpret the results of searches and measurements are introduced in the
...
referred to as \emph{channels}, defined by certain event selections. Each region
...
\item The term $f(a_p \mid \alpha_p)$ can be interpreted as the likelihood
  function resulting from an auxiliary measurement of the parameter~$\alpha_p$
  given that $a_p$ is observed. Conventionally, $f(a_p \mid \alpha_p)$ is taken
  to be the probability density of a Normal distribution with unit variance and
  mean $\alpha_p$. Furthermore, it is assumed that the measurement observed
  $a_p = 0$ such that, considering only the auxiliary measurement, the maximum
  likelihood estimate (MLE) of $\alpha_p$ is 0 with an uncertainty of
  $\Delta \alpha_p = \pm 1$.

  This particular choice of constraint term is intimiately connected to the
  functional form of $\sigma_{csb}(\myvec{\alpha})$ and
  $\eta_{cs}(\myvec{\alpha})$, which completely describes the physical effect of
  the nuisance parameter.


  sometimes a simplified representation of an actual measurement.

  sometimes a pseudo-measurement that serves to incorporate an uncertainty in
  the model.
