...
\item The $\pois(m_{cb} \mid \gamma_{cb} \tau_{cb})$ terms provide constraints
  ...
  measure of the statistical precision of the background estimate using a finite
  sample of events. This statistical uncertainty is incorporated in the model by
  setting up auxiliary measurements of the effective number of unweighted
  events, $\gamma_{cb}\tau_{cb}$, for every bin. These measurements are assumed
  to observe $m_{cb} = \tau_{cb}$, and therefore the nominal values of the
...
the expected experimental sensitivity of searches or measurements in HEP. The
methods that will be introduced in the following section,


The methods that will be introduced in the subsequent section can be applied to
Asimov datasets to yield median d
...
% In the
% subsequent section, statistical inference can be performed using Asimov dataa


% Asimov datasets yield the


% allow the determination of the median values
% of test statistics~\cite{Cowan:2010js},




% In
% the subsequent section, Asimov dataset


% can be used to obtain the median values of test statistics~\cite{Cowan:2010js},
% allowing to calculate median discovery significances or median upper limits on
% the signal strength parameter.


% median discovery significances and median upper limits on the signal strength.
...
of the \qmutilde-based test to reject $H_0$ when the background-only hypothesis
is true. This modification can be understood qualitatively by considering two
edge cases:
\begin{itemize}
...
\begin{figure}[htbp]
  ...
  \begin{subfigure}{0.46\textwidth}
    ...
    \caption{High signal sensitivity}
  \end{subfigure}\hfill%
  \begin{subfigure}{0.46\textwidth}
    \includegraphics[width=\textwidth]{statistics/cls_low_sensitivity}
    \caption{Low signal sensitivity}
  \end{subfigure}

  \caption{Qualitative illustration of the sampling distributions of \qmutilde
    under the signal-plus-background and background-only hypothesis. The red
    (blue) hatched area corresponds to the integrals entering the numerator
    (denominator) of the \CLs statistic in~\Cref{eq:cls}.}%
...
limits are estimated using the \CLs technique.
