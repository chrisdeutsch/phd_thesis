...
% Maybe say something about signals / backgrounds and define what a 'physics
% process' is?
...
inferred from the observed data, is parameterised as~\cite{cranmer2012}
\begin{align*}
...
\end{align*}
where $\myvec{\alpha}$, $\myvec{\phi}$, $\myvec{\gamma}$ are parameters of the
model, and $\mathcal{S}$ is the set of physics processes that are included in
the model. The parameters $\myvec{\alpha}$ and $\myvec{\gamma}$ are nuisance
parameters constrained by auxiliary measurements, which will be introduced
shortly. The relevance of the four factors $\gamma_{csb}$, $\phi_{cs}$,
$\eta_{cs}$, and $\sigma_{csb}$ is described in the
...
\item $\sigma_{csb}(\myvec{\alpha})$ is referred to as the \emph{parameterised
  ...
  process~$s$ in channel~$c$ and is usually derived from simulation or control
...
\item $\eta_{cs}(\myvec{\alpha})$ represents a normalisation factor applied
  ...
  freely since it is a function of the nuisance parameters~$\myvec{\alpha}$,
  which are themselves constrained by auxiliary measurements. This factor is
  included to account for uncertainties on the normalisation of process~$s$ in
  channel~$c$.

\item $\phi_{cs}$ represents a free normalisation factor applied uniformly to
  ...
  In most cases, at least one normalisation factor is present that
  is applied to the physics process of interest, also referred to as the
  \emph{signal}. This normalisation factor is referred to as the signal
  strength, $\mu$, and is often considered to be the parameter of interest
  (POI).

\item $\gamma_{csb}$ are parameters that introduce additional degrees of freedom
  for every channel, process, and bin. These parameters allow to incorporate
  ...
  account for these uncertainties. To reduce the number of parameters of the
  statistical model, the method is simplified as proposed in
  Ref.~\cite{conway2011} by only considering the statistical uncertainty on the
  sum of processes in a given bin/channel. Consequently, the parameters
  $\gamma_{csb}$ can be replaced by $\gamma_{cb}$, omitting the dependence on
  the physics process.

\end{itemize}


A discussion of the functional form of $\sigma_{csb}(\myvec{\alpha})$ and
$\eta_{cs}(\myvec{\alpha})$ is omitted here but can be found in
Ref.~\cite{cranmer2012}.


These parameters can be fixed or correlated
...
\begin{align*}
  ...
  \,\Biggr]
  \times C(\myvec{\alpha}, \myvec{\gamma})
\end{align*}


% \prod_{p \in \mathrm{A} \cup \Gamma} f_{p}(a_{p} \vert \alpha_{p})

\begin{align*}
  C(\myvec{\alpha}, \myvec{\gamma}) =
  \Biggl[\, \prod_{p \in A} f_p(a_p \vert \alpha_p)     \,\Biggr]
  \Biggl[\, \prod_{p \in \Gamma} f_p(g_p \vert \gamma_p) \,\Biggr]
\end{align*}


The choice of constraint terms is intimately connected to the meaning of
$\eta_{cs}$ and $\sigma_{csb}$.
