...
Given some observed data, the parameters of the model can be estimated using
...
comparisons of the background-only model with the signal-plus-background model
are commonly performed when searching for new phenomena. This comparison
procedure is formalised in the framework of \emph{statistical hypothesis
  testing.}
...
hypothesis,\footnote{Example: Taking the background-only hypothesis as the null
  ...
  = 0 \}$.}  and an alternative hypothesis, $H_1$, as
\begin{align*}
...
The \emph{likelihood ratio test} (LRT) is frequently used in HEP for hypothesis
testing.
...
$q_0 > q_{0, \text{crit}}$, where $q_{0, \text{crit}}$ is the chosen critical
value for the test, as an asymptotic significance of\todo{Not correct!}
\begin{align*}
  p_0 = \int_{q_{0, \text{crit}}}^\infty \mathrm{d}q_0 \, f(q_0; 1) =
  1 - \Phi\left(\sqrt{q_{0, \text{crit}}}\right)
  %\sqrt{q_0^\text{crit}} \,\text{.}
\end{align*}
