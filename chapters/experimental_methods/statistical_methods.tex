...
on~$p_\mu$ is illustrated. A test of $H_0: \mu \geq \mu^*$ and
...
rejecting~$H_0$ under the background-only hypothesis, $1 - \beta$, is solely
determined by the experimental sensitivity to a signal with
...
The \CLs technique introduces the alternative test statistic~\cite{Cowan:2010js}
\begin{align*}
  \CLs = \frac
...
observing a \qmutilde larger than the observed value under the background-only
hypothesis. Upper limits on the signal strength are then estimated by finding
the largest value of $\mu$ for which $\CLs > \alpha$. In the limiting case of an
experiment with no signal sensitivity the value of \CLs approaches unity, thus
preventing spurious exclusions.

\vspace{5em}

These limits are also referred to as upper limits at $1 - \alpha$~CL, however,
their coverage probability exceeds $1 - \alpha$ by construction due to
$\CLs > p_\mu$. Since \CLs approaches unity in cases of low signal sensitivity,
spurious exclusions are effectively prevented when using the \CLs
method. Conventionally, upper limits are set at $\SI{95}{\percent}$~CL
($\alpha = 0.05$) in HEP.


Finally, the \emph{median
