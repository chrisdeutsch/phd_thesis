...
In searches for new physics, the
\CLs~technique~\cite{Junk:1999kv,Read:2000ru,Read:2002hq} is used to set upper
limits on $\mu$. To motivate the use of this method, a problem with tests based
on~$p_\mu$ is illustrated hereafter. A test of $H_0: \mu \geq \mu^*$ and
$H_1: \mu < \mu^*$ (NPs are omitted for brevity) with rejection
...
Since $\alpha$ is fixed a priori, the probability (statistical power) of
correctly rejecting~$H_0$ under the background-only hypothesis, $1 - \beta$, is
...
experiment that has hardly any sensitivity in distinguishing between the
$\mu = \mu^*$ and $\mu = 0$ hypotheses. In this case, the statistical power of
the test approaches $\alpha$, which is illustrated
in~\Cref{fig:cls_qmu_lowpower}. This means that a signal to which the experiment
has little sensitivity will still be excluded with a probability of at
least~$\alpha$ under the background-only hypothesis. In HEP these \emph{spurious
  exclusions} are undesired, motivating the introduction of an alternative test
statistic for upper limits that takes into account the sensitivity of the
experiment.
...
The \CLs technique addresses the spurious exclusion of signals by introducing
...
\begin{align*}
  ...
  % \label{eq:cls}
\end{align*}
where $\tilde{q}_{\mu, \text{obs}}$ is the observed value of \qmutilde. The \CLs
statistic modifies $p_\mu$ (the numerator) by dividing it by the probability of
having a \qmutilde larger than the observed value under the background-only
hypothesis. This modification can be understood qualitatively by considering the
two limiting cases:
\begin{itemize}

\item High signal sensitivity (high statistical power): $\CLs \to p_\mu$.

\item Low signal sensitivity (low statistical power): $\CLs \to 1$.

\end{itemize}
The \CLs technique estimates upper limits on the signal strength by finding the
largest value of $\mu$ for which $\CLs > \alpha$. Since \CLs approaches unity in
cases of low signal sensitivity, spurious exclusions are effectively
prevented. These upper limits

\vspace{5em}


Upper limits derived from the condition
...
construction due to $\CLs \geq p_\mu$.


Conventionally, upper limits are set at
