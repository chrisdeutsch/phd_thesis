...
\begin{align*}
  \hat{\hat{\theta}}(\mu^\prime) = \argmax_{\myvec{\theta}} L(\mu^\prime, \myvec{\theta}) \,\text{.}
\end{align*}
...
The \emph{likelihood ratio test} (LRT) is frequently used in HEP to compare the
likelihood of two hypotheses given observed data. Let $\Omega$ be the parameter
space of the statistical model introduced in the previous section. A null
hypothesis, $H_0$, is defined as
\begin{align*}
...
hypothesis,\footnote{Example: If one is interested in taking the background-only
  hypothesis as the null hypothesis (and no restrictions are made regarding the
  possible values of the NPs), then
  $\Omega_0 = \{ (\mu, \myvec{\theta}) \vert (\mu, \myvec{\theta}) \in \Omega
  \text{ and } \mu = 0 \}$.} and an alternative hypothesis, $H_1$, as
\begin{align*}
...
\subsubsection{Discovery of a Positive Signal}
