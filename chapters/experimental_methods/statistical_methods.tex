...
by construction. Thus, the probability of incorrectly rejecting $H_0$ when
$\mu = \mu^*$ is $\alpha$. Since $\alpha$ is fixed a priori, the probability
(statistical power) of correctly rejecting $H_0$ assuming the background-only
hypothesis
% , which can range from $\alpha$ to one,
...
the power of the test has implications on how statistically significant tests
have to be interpreted.




Consider the probability of~$H_1$ being true given that the test is significant
(positive).
...
the prior probabilities that $H_1$ is true and false, respectively.
