...
\emph{discovery significance}~$Z_0$, which is defined as
\begin{align*}
...
\num{2.87e-7}$). This threshold is also referred to as the ``$5\sigma$ threshold
for discovery''.


\subsubsection{Upper Limits on the Signal Strength}

In the absence of a statistically significant signal, one is often interested in
setting an upper limit on the signal strength parameter that is still compatible
with the observations. The relevant hypotheses for setting upper limits on $\mu$
are
\begin{align*}
...
\end{align*}
where $\mu^*$ is an arbitrary constant and it is assumed that the signal
strength is non-negative. A test statistic based on the LRT is
constructed~\cite{Cowan:2010js}
\begin{align*}
  \tilde{q}_{\mu} =
  \begin{cases}
