...
\item The term $f(a_p \mid \alpha_p)$ can be interpreted as the likelihood
  ...
  $\eta_{cs}(0)$ represents the nominal histogram and normalisation, while
...
\item The $\pois(m_{cb} \mid \gamma_{cb} \tau_{cb})$ terms provide constraints
  ...
  bin and $w_i$ corresponds to the event weight of the $i$-th event. This
  constant is interpreted as an effective number of unweighted
  events.\footnote{Let $Y_{\text{w}} = \sum_{i = 1}^{N} W_i$ and
    ...
    after approximating the expectations with sample averages
    $\expect(N^\prime) \approx ( \sum_i w_i )^2 / \sum_i w_i^2$.}
  ...
  $m_{cb} = \tau_{cb}$
    ...
    % (i.e.\
  % $\sqrt{\var(Y_{\text{w}})} / \expect(Y_{\text{w}}) = \sqrt{\var(Y)} /
  % \expect(Y)$).
