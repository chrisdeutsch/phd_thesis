...
consists of three fully-connected (\emph{dense}) layers with 128, 128, and 16
neurons each. All layers include trainable bias vectors and use the
...
sequences of tracks and clusters are processed by two fully-connected
layers with shared weights (\emph{shared dense}) with 32 neurons each,
trainable bias vectors, and using ReLU activation
functions. \emph{Shared dense} layers map sequences of input vectors,
$(\myvec{x}_i)_{i=1}^{n}$, to sequences of output vectors of the same
length, $(\myvec{y}_i)_{i=1}^{n}$, using a transformation of the form
\begin{align*}
...
sequence. As a result, the \emph{shared dense} layers produce
intermediate presentations of the sequences of tracks and clusters
which are subsequently passed to recurrent layers.
