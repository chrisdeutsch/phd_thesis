...
\begin{table}[htbp]
  ...
  \caption{Summary of allowed \klambda intervals obtained from
    ...
    from the upper limits obtained from Asimov datasets with
    $\xsecggfvbf = 0$.}%
...
\bbtautau channel (cf.~\Cref{sec:dihiggs}) in terms of anomalous Higgs
boson self-coupling strengths was presented. Upper limits at
\SI{95}{\percent} CL were set on the total cross section of Higgs
boson pair production via \ggF and VBF as a function of the
self-coupling constant. An allowed range of $\klambda \in [-2.4, 9.2]$
was obtained by comparing the cross section exclusion limits with
cross section predictions from theory. The results are based
assumptions that other coupling constants are as predicted by the SM
and neglecting changes in single Higgs boson production cross sections
and Higgs boson branching ratios with \klambda due to electroweak
corrections.

A follow-up analysis was performed by the ATLAS collaboration in
Ref.~\cite{ATL-HDBS-2022-03-002} combining the three most sensitive
channels to non-resonant \HH production, namely the \bbtautau, \bbbb,
...
parameter of interest which allows to obtain confidence intervals on
\klambda using likelihood-ratio tests. Moreover, the
\klambda-dependency of single Higgs boson production and the Higgs
boson decay were accounted for. The combination yields an observed
confidence interval of $\klambda \in [-0.6, 6.6]$ (expectation of
$\klambda \in [-2.1, 7.8]$ for the $\klambda = 1$ hypothesis) computed
at \SI{95}{\percent} CL~\cite{ATL-HDBS-2022-03-002}. The \klambda
confidence intervalbased on the \bbtautau search only is
$\klambda \in [-2.7, 9.5]$ at \SI{95}{\percent} CL showing reasonable
agreement with the result obtained in this chapter using simplifying
assumptions.

Prospects for \klambda constraints at the HL-LHC were


\cite{ATL-PHYS-PUB-2021-044,ATL-PHYS-PUB-2022-001,ATL-PHYS-PUB-2022-005}
