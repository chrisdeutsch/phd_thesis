...
\begin{align*}
  \lagrange_{\text{mass term}}^{\text{vector boson}} = -\frac{1}{2} m^2 A_\mu A^\mu
\end{align*}
...
\begin{align*}
  \lagrange_{\text{mass term}}^{\text{fermion}} = - m \bar{\psi} \psi
\end{align*}
...
...
Two complex scalar fields, one charged and one neutral field, are introduced
that are arranged in a weak isospin doublet with $Y = 1$ according to
\begin{align*}
  \phi =
  \begin{pmatrix}
    \phi^+ \\
...
\end{align*}
with two parameters $\mu^2$ and $\lambda$.\todo{Maybe say that this is the only
  form that can be motivated theoretically? (higher powers do not yield
  renormalisable theories)} The potential must be bound from below to have a
...
choice is further illustrated in \Cref{fig:mexican_hat} for a simplified model
...
$SU(2) \otimes U(1)$ is an exact symmetry of the Lagrangian, however, due to the
non-vanishing VEV the symmetry is broken by the vacuum state.



``Nature chooses one of the degenerate states'' (though they are related by the
symmetry).
...
Goldstone theorem~\cite{Goldstone:1962es} / Unitary gauge
