...
\begin{align*}
  ...
  %\label{eq:barlow_beeston_llhconstraint}
\end{align*}
...
% Finally, the expectation of $X$ and $\tilde{X}$ can be expressed as
% \begin{align*}
%   \expect(X) = \expect(\tilde{X}) = \gamma_{cb} \sum_i w_i =
%   \gamma_{cb} \nu_{cb} \,\text{,}
% \end{align*}
% where $\nu_{cb} = \sum_i w_i$ is the background rate predicted using the
% finite sample of events, thus motivating the substitution
