...
equal to the mean number of events predicted by the background model.
...
in the background estimate.\footnote{In the absence of negatively weighted
  ...
  $\pois(w_i)$ for every event with weight~$w_i$ used for background
  estimation.} Instead, a model based on the mathematical framework provided by
Sklar's theorem~\cite{Sklar1959FonctionsDR} is adopted, which can be used to
...
functional form of the copula since the marginal distributions are already known
from the nominal background estimate. The Poisson distribution can be
