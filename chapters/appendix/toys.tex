The following describes the toy generation method used for the global
significance estimation in \Cref{sec:global_significance}.
...
\end{align*}
This equation reflects the intuition that the overlap in selected kinematic
region, described by $\lambda_2$, induces a correlation between both
observables. In the following, the model described by $A$ and $B$ is referred to
as the bivariate Poisson model~\cite{teicher1954}.
...
\Cref{fig:correlation_matrix_observables} shows estimated correlation
coefficients between observables in the \hadhad channel. A few features of the
correlation matrix are discussed in the following:
\begin{itemize}
...
\item The observables of the most signal-like bins are expected to have little
  ...
  range considered in~\Cref{fig:correlation_matrix_observables_medium}.  This is
  in contrast to the correlation of the most signal-like bins of the high-\mX
  discriminants shown in \Cref{fig:correlation_matrix_observables_high}. In this
...
distributions of the observables, i.e.\ Poisson distributions with parameters
equal to the mean number of events predicted by the nominal background model.
...
in the background estimate.\footnote{In the absence of negatively weighted
  events, toy experiments can be generated by drawing a random weight from
  $\pois(w_i)$ for every event used for background estimation with event
  weight~$w_i$.} Instead, a model based on the mathematical framework provided
by Sklar's theorem~\cite{Sklar1959FonctionsDR} is adopted, which can be used to
factorise the marginal distributions from the dependency structure of the
observables. A review of similar approaches of modelling multivariate count data
is given in Ref.~\cite{10.1002/wics.1398}.

Sklar's theorem states~\cite{nelsen} that any $n$-dimensional joint distribution
function $H$ can be decomposed into $n$ marginal distribution functions
...
\item The conditional variance of $B$ given $A = a$ illustrates that both models
  ...
  by up \SI{15}{\percent} to between both models.
  % These deviations are assumed to be negligible compared to the linear trend
  % that is properly captured by the model.

\item In the case of disjoint or fully overlapping bins (not shown in the
  figure) the bivariate Poisson model and copula-based model are identical.
...
generated using the copula-based model. The following algorithm is employed to
draw a vector of random variates $\myvec{x} = (x_1, \dots, x_n)$ from the joint
distribution described by the Gaussian copula and marginal distributions:
\begin{enumerate}
...
\item Draw a vector of random variates $\myvec{u} = (u_1, \dots, u_n)$ from the
  ...
  transformations are provided by a singular value decomposition of $\myvec{R}$.
...
Barlow--Beeston method (cf.~\Cref{app:barlow_beeston}).
%
...
$\pois(\lambda = 1)$ is assigned to every event, serving as an approximation to
the regular non-parametric
...
follows from the SPD approximation of the CPD employed by the Barlow--Beeston
method. Multiple toy experiments are generated by repeating the Bootstrap
procedure.
