...
Calibrations of the initial estimate of the \tauhadvis candidate four-momentum
to the \tauhadvis energy scale (TES) are derived using simulated events
containing hadronic \taulepton decays. Two calibration methods are used:
\begin{description}

\item[Calorimeter-based TES] First, the expected energy contribution of pile-up
  to the \tauhadvis is subtracted. The pile-up subtraction is parameterised as a
  linear function of the number of reconstructed PVs in the event and in bins of
  \tauhadvis $|\eta|$ and \Ntracks. Afterwards, the detector response is
  calibrated as a function of the energy of the \tauhadvis after pile-up
  subtraction and in bins of \tauhadvis $|\eta|$ and \Ntracks.

\item[MVA-based TES] In the region from \SIrange{200}{300}{\GeV} the MVA-based
  TES smoothly transitions into the calorimeter-based TES.
