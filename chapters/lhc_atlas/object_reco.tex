...
defined four-momenta (e.g.\ topo-clusters in the calorimeters). Any set of one
or more entities can be viewed as a pseudo-jet with a four-momentum given by the
sum of the constituent four-momenta. Pseudo-jets are sequentially combined if
they are close according to a distance metric. The distance between the
...
\item If $\min_{i,j}(d_{ij}) \geq \min_{i}(d_{i\text{B}})$: The pseudo-jet
  ...
  procedure.
\end{enumerate}
These steps are repeated until no more pseudo-jets are left.
...
Idea: Use trackers to determine the momenta of charged hadrons in jets due to
superior momentum resolution of the tracking system at low transverse
momenta. Neutrals are estimated from the calorimeter. To avoid double-counting
of the energy of charged hadrons, the contribution of charged hadrons to the
calorimeter energy has to be subtracted.

Can access charged particles of lower momenta which would not pass the noise
threshold of the topo-cluster algorithm.

Better angular resolution of the tracker.

Tracks can also be matched to the PV allowing to subtract the contribution of
pile-up to the energy of reconstructed jets.
...
\begin{align*}
  \frac{\sigma(1 / \pT)}{1 / \pT} = a \cdot \pT \oplus b
\end{align*}
...
\cite{PERF-2015-09,JETM-2018-05}
