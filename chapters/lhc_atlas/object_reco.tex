...
$\myvec{p}_{\text{T}}^{\text{undetected}}$ are the total transverse momentum
vector of all detected and undetected particles produced in the hard
interaction, respectively. Therefore, the missing transverse momentum is defined
as
\begin{align*}
  \pTmiss
...
or only weakly-interacting particles such as neutrinos. An object-based
definition of \pTmiss from Ref.~\cite{PERF-2016-07} is used that is defined as
\begin{align}
  \pTmiss =
  - \sum \pTvec{e}
  - \sum \pTvec{\gamma}
  ...
  \label{eq:met}
\end{align}
where the $\myvec{p}_{\text{T}}$-sums go over selected electrons, photons,
muons, \tauhadvis, and jets. An additional term, \pTvec{\text{soft}}, is
introduced to account for soft radiation that is not reconstructed as part of
another object. This term, referred to as the track soft term, is estimated by
summing the transverse momenta of all tracks that pass basic track quality
criteria and cannot be associated to any selected electron, photon, muon,
\tauhadvis, or jet.

The difficulties in reconstructing the total transverse momentum of non- or only
weakly-interacting using \pTmiss is illustrated by \Cref{eq:met}. Any objects
that were not included in the calculation, either due to detector acceptance
effects or inefficiencies in the reconstruction, will distort the estimated
\pTmiss

. Moreover, uncertainties on the

of individual objects accumulate, leading to large uncertainties on \pTmiss

particularly in events with high activity.
...
Therefore, \pTmiss is only a crude estimate with uncertainties strongly
depending on the event activity.


sums electrons, photons, muons, \tauhadvis, jets. An additional term


\pTvec{\text{soft}} is introduced accounting for soft radiation that is not
reconstructed as any object.
...
Difficulties in \pTmiss reco:
- All particles need to be detected
- Uncertainties on transverse momenta of all objects accumulate
...
non- or only weakly-interacting such as neutrinos
