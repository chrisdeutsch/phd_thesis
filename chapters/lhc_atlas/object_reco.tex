...
The large solid angle coverage and detector subsystems of the ATLAS detector
allows to reconstruct most objects produced in the hard interaction, allowing
for an almost full interpretation of the event. Particles are reconstructed from
their signatures in the detector



Electrons, muons, \tauleptons, jets are reconstructed from their signatures in
the detector.

- Reconstruction of physics objects based on their signals in the detector

- Same reconstruction techniques are applied to simulated events and data
recorded with the detector

- First, low-level objects such as tracks, vertices and topo-clusters.

- Second, high-level physics objects reconstructed from the lower-level ones.

- Finally \pTmiss



A description of photon reconstruction or specialised reconstruction techniques
is omitted. Details on the object selection used for the studies presented in
\Cref{sec:tauid,sec:dihiggs} are given in the respective chapter.
