...
as leptonic and hadronic decay modes, respectively.
...
signature of their visible decay products, neglecting the neutrinos produced in
...
are used to identify tracks originating from charged pions produced in
\taulepton decays. These tracks are referred to as \emph{core} tracks, since
they are usually found in the \emph{core region} of $\Delta R < 0.2$ about the
\tauhadvis axis. The number of \emph{core} tracks associated to a \tauhadvis
...
\begin{align*}
  & \pT > \SI{1}{\GeV} && |d_{0}| < \SI{1}{\milli\metre} && |z_{0} \sin\theta| < \SI{1.5}{\milli\metre} \\
  & N_{\text{pixel}} \geq 2 && N_{\text{pixel}} + N_{\text{SCT}} \geq 7 \,\text{,}
\end{align*}
where the track parameters are given at the perigee with respect to the... The
number of hits on the reconstructed track, counting dead sensor modules located
on the trajectory, is given by $N_{\text{pixel}}$ and $N_{\text{SCT}}$,
respectively.
