...
several points on the trajectory, and the known magnetic field in the ID, is
used to reconstruct the trajectory of the particle. The reconstructed trajectory
is referred to as the \emph{charged-particle track} and often abbreviated as
...
resolution. This facilitates the removal of tracks from pile-up, which are
typically displaced from the PV along the $z$-axis. Moreover, the accurate
reconstruction of secondary vertices, i.e.\ displaced vertices originating from
decays of unstable particles produced in the hard interaction, is crucial to
identify jets originating from
...
\begin{figure}[htbp]
  ...
  \begin{subfigure}[b]{0.45\textwidth}
    % trim args: left bottom right top
    \includegraphics[width=\textwidth, trim=1.1cm 1cm 3.2cm 1cm,
    clip]{atlas/atlas_indet_2_with_ibl}%
    \subcaption{}%
...
barrel region of the ATLAS detector (cf.\ \Cref{fig:atlas_inner_detector}). The
...
\Cref{fig:atlas_calorimeters}, surrounds the ID and is used for destructive
...
missing transverse momentum, which is discussed in \Cref{sec:atlas_met}.
% % or non- or only weakly-interacting particles from BSM theories,
...
The ATLAS calorimeter consists of two major parts. The \emph{electromagnetic
...
hadrons.
% that are able to pass through the electromagnetic calorimeter.
