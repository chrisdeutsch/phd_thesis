...
The MS, schematically depicted in \Cref{fig:atlas_muon_system}, is the outermost
and largest part of the ATLAS detector.


It is a tracking detector used to
provide an independent momentum measurement of muons that can pass the
calorimeters

\begin{itemize}
\item Outermost and largest
\item Tracking detector
\item Independent momentum measurement for muons (can pass calorimeters)
\item Different techologies: gaseous ionisation detectors which allow to cover
  large areas while being relatively cheap.
  \begin{itemize}
  \item RPC (resistive plate chambers): Very fast -> triggers
  \item CSC (cathode strip chambers): crossed cathode strips and anode wires
  \item TGC (thin-gap chambers):
  \item MDT (monitored drift tubes):
  \end{itemize}
\end{itemize}
...
Principally it is a tracking detector measuring
