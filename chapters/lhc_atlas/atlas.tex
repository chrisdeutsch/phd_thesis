...
toroid magnets that bend the trajectories of muons in or against the
$z$-direction. The bending of charged particle trajectories in the ID and MS
...
avoiding detection, which consequently allows to determine the transverse
...
\begin{figure}[htbp]
  ...
  \caption{Overview of the ATLAS calorimeter system. Image taken from
    Ref.~\cite{PERF-2007-01}.}%
...
at nuclei, spallation, fission, or neutron capture\todo{citation?}. Moreover,
electromagnetic showers are an important sub-component of hadronic showers
which, for example, originate from photons produced in $\pi^0$ decays. The
length scales of hadronic showers are larger, both laterally and longitudinally,
than the ones of electromagnetic showers.
...
endcap ($1.375 < |\eta| < 3.2$), and forward ($3.1 < |\eta| < 4.9$)
region~\cite{PERF-2007-01}. The electromagnetic calorimeters in the barrel and
endcap region consist of lead absorbers with LAr-filled gaps that are
...
barrel ($|\eta| < 1.0$), extended barrel ($0.8 < |\eta| < 1.7$), endcap
...
hadronic endcap calorimeter (HEC) and forward calorimeter consist of LAr-filled
...
instrumentation of a large areas. The MS provides a coverage of $|\eta| < 2.7$
for precision tracking and $|\eta| < 2.4$ for triggering on muons.
...
\begin{figure}[htbp]
  ...
  \caption{Overview of the ATLAS muon spectrometer. Image taken from
    Ref.~\cite{PERF-2007-01}.}%
  \label{fig:atlas_muon_system}
\end{figure}

The ATLAS detector uses multiple layers monitored drift tube chambers
\footnote{Deformations of the muon chambers are actively monitored using an
  optical alignment system built into the frame of the chambers.} (MDTs) for
...
($|\eta| < 1.05$) and endcap ($1.05 < |\eta| < 2.7$) region, respectively,
...
beamline in the barrel region. The endcap region is instrumented by muon
chambers in the form of three wheels perpendicular to the beampipe. A
\emph{small wheel} is located in front of the endcap toroid, and two \emph{big
  wheels} are located behind the endcap toroid. Every layer/wheel consists of
...
The task of the ATLAS trigger system is to reduce the rate at which events are
recorded by performing a pre-selection already during data-taking. This is
achieved by monitoring the ATLAS detector for signatures of rare physics
processes, such as the presence of charged leptons with large transverse
momenta.

-> Heavily constrainted due to the limited detector read-out and latency and
bandwidth constraints on the algorithms running at trigger-level.

During Run~2, the trigger system reduced the rate of recorded events to about
\SI{1000}{\hertz} which
...
\item Triggers are very much limited by the maximum allowed latency and
  bandwidth (trigger rate).

\item The ATLAS trigger system is divided into two stages. The first stage is
  the Level-1 (L1) trigger. -> target \SI{100}{\kilo\hertz}

\item The second stage is the High-Level trigger (HLT) -> target: \SI{200}{\hertz}
