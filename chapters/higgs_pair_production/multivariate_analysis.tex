...
2~$b$-tag pre-selection region of the \hadhad channel are used to
train the BDTs. The training uses simulation of the non-resonant \HH
...
\todo[inline]{Performance? ROC and distribution of SMBDT. -> Could
  connect back to ``three orders of magnitude''.}
...
\begin{figure}[htbp]
  ...
  \begin{subfigure}[t]{.48\textwidth}
    \missingfigure{SMBDT score}
  \end{subfigure}\hfill %
  \begin{subfigure}[t]{.48\textwidth}
    \missingfigure{ROC-AUC?}
  \end{subfigure}

  \caption{ROC (?) and SMBDT distribution}
\end{figure}

To understand how the BDT uses the input variables the permutation
importance is used \Cref{tab:variable_importance}. To extract the SM
signal the invariant masses of the two Higgs boson candidates is most
important.\todo{Move ROC-AUC to variable importance part?}

\begin{table}[htbp]
  \centering

  \missingfigure{Variable importance}
