...
invariant mass of the $H \to \bbbar$ or $H \to \tautau$ candidate. A
number of reconstructed quantities can be defined that offer
discrimination power between the various signals and backgrounds.
...
able to handle sets of classification tasks described by one or more
...
used for training or model selection (the process of choosing a
model), thus providing an estimate of the generalised performance. In
...
In this search, model selection is performed using 5-fold cross
validation (CV), a generalisation of the 2-fold approach to a larger
number of subdivisions, separately on even- and odd-numbered
events. This approach effectively nests 5-fold CV inside of 2-fold CV
and is therefore called \emph{nested cross
...
deviation of a evaluation metric over the five iterations of the inner
...
are reconstructed\footnote{The fraction of events with a
  ...
  the \btagged jets / \tauhadvis not being matched to Higgs boson
...
\item[\mHH] The invariant mass of the pair of Higgs bosons in signal
  events. It is determined from the sum of four-momenta of \btagged
  jets after momentum corrections and the $\tautau$-system
  four-momentum reconstructed using the MMC. In signal events the
  relative mass resolution is \SIrange{8}{10}{\percent}.

\end{description}
The performance of the \PHiggs and \HH mass reconstruction is
...
For a given analysis channel the same input variables are used for the
non-resonant and resonant \HH search. The choice of variables between
channels differs and is summarised in~\Cref{tab:mva_inputvar}. A
description of variables used in the \lephad channels is given in the
following.  Reconstructed electrons and muons are collectively
referred to as leptons ($\ell$).\todo{Move lephad variables to
  appendix?}
\begin{description}
...
\begin{table}[htbp]
  ...
  \caption{Discriminating variables used by the multivariate
    ...
    and resonant production modes. Adopted from
...
The extraction of SM \HH signal events uses BDT for binary
classification, distinguishing signal from background events. Events
in passing the signal region selection (2~$b$-tag region) of the
\hadhad channel are used to train BDT. The training uses simulation of
the non-resonant \HH production via \ggF as the \emph{signal class};
the combination of all backgrounds considered in the analysis as the
...
weighted according to their relative cross-sections. The SM \HH
production via VBF is not included due to the its low cross-section
compared to the \ggF production mode. The BDT implementation of the
...
tree branching 400 cut values on input variables are considered. Other
settings remain at their default
...
\begin{table}[htbp]
  ...
  \caption{Hyperparameter values considered for the random grid search
    used to optimise the performance of the BDT extracting the
...
\todo[inline]{Have some rejection power vs mass plots?}
