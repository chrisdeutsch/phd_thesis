...
\begin{figure}[htbp]
  ...
  \caption[Distribution of the PNN discriminants in the \hadhad
  SR.]{Distribution of the PNN discriminants evaluated with PNN mass parameters
    ...
    ($\mX = \SI{1000}{\GeV}$) score will be discussed in
...
\begin{table}[htbp]
  ...
  \caption[Permutation importance of the input variables to the
  PNN.]{Permutation importance of the input variables to the PNN measured as the
    change in ROC-AUC (for the binary classification task between a signal with
    mass \mX and total background) when permuting the values of a single
    variable over all events. The mean $\Delta\text{ROC-AUC}$ over 10
    permutations is displayed. The statistical uncertainty is below 0.002 for
    (a) and 0.001 for (b) and (c) and thus omitted.  Variables are ordered by
