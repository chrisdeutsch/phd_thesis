...
describes the event classification using PNNs, including a discussion
...
for the non-resonant and resonant \HH search. The choice of variables
between channels differs and is summarised
in~\Cref{tab:mva_inputvar}. A description of variables used in the
\lephad channels is given in the following.  Reconstructed electrons
and muons are collectively referred to as leptons ($\ell$).\todo{Move
  lephad variables to appendix?}
\begin{description}
...
via VBF is not included in training.
...
(excluding) the SM \HH production via vector boson fusion, similar to
the estimates from cross validation during model selection.
...
estimator, derived from a importance measure introduced in
...
the total background with a minor contribution from multi-jet.  The
...
in the SM which has a mean reconstructed \mHH of \SI{500}{\GeV} after
the signal region selection. For high mass resonances, the most
...
constraints. The algorithm will be formally introduced
...
signal and background events, respectively. A combined signal
significance is obtained by adding the bin-wise significances in
quadrature.
