...
\Cref{sec:experimental_uncertainties} but exclude uncertainties
related to the \jettotauhadvis estimation in the \hadhad channel which
were previously described in
\Cref{sec:bkg_hadhad_ttbarfakes,sec:bkg_hadhad_ff}. Theory
uncertainties are described in~\Cref{sec:theory_uncertainties} and
...
uncertainty is assigned to all predictions normalised using
theoretical cross section predictions.  Moreover, an uncertainty is
assigned on the re-weighting used to match the instantaneous
luminosity distribution used to generate the pile-up overlay with the
conditions of the recorded \pp-collision dataset.

Dedicated calibration measurements performed for
...
the \pTmissAbs reconstruction~\cite{ATLAS-CONF-2018-023} by the ATLAS
collaboration.\footnote{In some cases publications of the calibrations
  measurements used in this search are not available at the time of
  writing. In these cases earlier, representative measurements are
  referenced.} These measurements provide calibrations of, where
applicable, the momentum scales and resolutions, and efficiencies of
various selection steps such as the identification and isolation
requirements. The six major instrumental uncertainties affecting
reconstructed physics objects are summarised in
\Cref{tab:experimental_uncertainties_2}.

Where applicable, dedicated calibrations and uncertainties are used
for simulated event samples using \emph{fast simulation} of the ATLAS
detector.
...
Except for the luminosity uncertainty, any instrumental uncertainty
can affect the normalisation and shape of the distribution of signals
and backgrounds in the final discriminant. These are propagated and
...
\begin{table}[htbp]
  ...
  \caption{Summary of instrumental uncertainties. The number of
    ...
    $N_{\text{d.o.f.}}$ in the right-most column. $\dagger$:~Due to
    the use of the object-based \pTmissAbs definition, uncertainties
    affecting reconstructed objects entering the \pTmissAbs estimation
    are also propagated as uncertainties to \pTmissAbs.}%
...
% \begin{description}
% \item[Integrated luminosity and pile-up] The integrated luminosity
%   of the $pp$ collision dataset collected with the ATLAS detector in
%   the period from 2015 to 2018 (\SI{139}{\per\femto\barn}) is
%   measured with an uncertainty of
%   \SI{1.7}{\percent}~\cite{ATLAS-CONF-2019-021}. This uncertainty is
%   assigned to all processes estimated using simulation that are
%   normalised using theoretical cross section.

% \item[Electrons] Uncertainties on electrons in simulation are
%   obtained from dedicated calibration
...
%   reconstruction, identification, isolation, and trigger
%   efficiencies in simulation are obtained from measurements
%   in~$J/\psi \to e^+e^-$ and $Z \to e^+e^-$ events and applied as
%   alternative weights to electrons in simulation.
% \end{description}
...
Uncertainties of the \faketauhadvis estimation in \lephad.
...
developed by the ATLAS collaboration:\todo{Move to appendix}
\begin{description}
