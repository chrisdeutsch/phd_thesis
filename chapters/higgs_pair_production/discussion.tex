...
The sensitivity of the \bbtautau channel to resonant production of Higgs boson
pairs via narrow-width, scalar resonances is complementary to the \bbyy and
\bbbb channels. A comparison of upper limits on $\sigma(\pp \to X \to \HH)$
between the three channels is shown in \Cref{fig:resonant_hh_limits} for
searches performed by the ATLAS collaboration using the full Run~2 \pp~collision
datasets. The \bbtautau channel has the highest expected sensitivity in the
intermediate mass range from \SIrange{375}{800}{\GeV}. The \bbyy channel yields
the most stringent upper limits for $\mX < \SI{375}{\GeV}$ due to its use of
di-photon triggers and excellent background rejection based on the invariant
di-photon mass. The highest expected sensitivity to signals with
$\mX > \SI{800}{\GeV}$ is provided by the \bbbb channel due to the large
$H \to \bbbar$ branching ratio, $b$-jet triggers becoming increasingly efficient
at selecting events with large \mHH, and the use of special reconstruction
techniques to reconstructed events with highly boosted Higgs bosons in a mass
range of $\SI{900}{\GeV} \leq \mX \leq \SI{5}{\TeV}$.
...
\begin{figure}[htbp]
  ...
  \caption{Expected and observed upper limits on $\sigma(\pp \to X \to \HH)$ at
    ...
    channel~\cite{HDBS-2018-40}. The searches were performed using
    \SIrange{126}{139}{\per\femto\barn} of \pp~collision data recorded during
    Run~2 of the LHC.}%
