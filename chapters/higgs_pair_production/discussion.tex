...
\item More events populate the signal-like region of the MVA discriminants as
  ...
  to the constraints on the minimum expected number of background events that
  have to be fulfilled by all bins. Consequently, as the integrated luminosity
  increases, the re-binning algorithm yields narrower bins in the most
  signal-like region of the MVA discriminants, which allows to better exploit
  the distinct signature of Higgs boson pair production.
  % Probably should put a number on this?

\end{itemize}

An outlook on the future of searches for Higgs boson pair production is provided
by extrapolations of SM~\HH searches to the conditions after data-taking at the
HL-LHC. The HL-LHC aims to deliver an integrated luminosity of at least
\SI{3000}{\per\femto\barn} at $\sqrt{s} = \SI{14}{\TeV}$~\cite{hllhc:tdr}, which
is the target of the extrapolations. The SM~\HH search presented in this chapter
was extrapolated to HL-LHC conditions by the ATLAS collaboration in
Ref.~\cite{ATL-PHYS-PUB-2021-044}. This extrapolation is conducted under the
assumption that the performance of the ATLAS detector can be maintained at the
level of the Run~2 performance in spite of the increase in instantaneous
luminosity at the HL-LHC. In addition, the evolution of systematic uncertainties
follows the recommendations in Refs.~\cite{ATL-PHYS-PUB-2019-005}. Under these
assumptions the search for SM~\HH production in the \bbtautau channel is
extrapolated to yield an expected discovery significance of $2.8\,\sigma$ under
the SM hypothesis~\cite{ATL-PHYS-PUB-2021-044}.
...
A similar extrapolation was performed in the \bbyy channel that yields an
expected discovery significance of $2.2\,\sigma$ at the end of the
