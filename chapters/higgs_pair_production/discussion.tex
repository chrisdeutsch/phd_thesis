...
\item The increase in integrated luminosity of the collected dataset
  improves the population of signal-like phase spaces allowing for
  better exploitation of the distinct signature of Higgs boson pair
  production.

  This is a side-effect of the limits imposed on the re-binning
  algorithms applied to MVA discriminants. The limits on the minimum
  number of background events (expected) and maximum statistical
  uncertainty on the background estimate for a good bin introduce a
  dependency of the re-binning algorithm on the integrated luminosity
  of data and simulation. As a result, an increase in integrated
  luminosity allows the most signal-like bin to be narrower which in
  the present case (due to the large discrimination power of the SM
  \HH MVA discriminants) improve the signal sensitivity.

  \todo{It would be good to know how large this effect is?}
\end{itemize}
