...
in~\Cref{sec:object_reconstruction,sec:event_selection}. Events with same
...
for Higgs boson pair production in $\bbbar\Plp\Plm$ final states with missing
transverse momentum.
...
% control region selection more similar to the selections applied for the signal
...
in~\Cref{tab:zcr_yields}. The majority of events in the control region originate
from \ZHF\footnote{About \SI{90}{\percent} of events in the \ZHF category are
  $Z + bb$ events.} or top quark pair production.
% Only about
...
\begin{table}[htbp]
  ...
  \input{tables/zcr_pre_and_postfit_yields}
\end{table}
...
\begin{figure}[htbp]
  ...
  \caption{Distribution of the invariant di-lepton mass for the combination of
    ...
    % at most one candidate is matched to heavy flavour quarks at
    % generator-level.
...
in~\Cref{sec:uncertainties,sec:statistical_analysis}. Restricting the maximum
...
systematic uncertainties. \Cref{tab:zcr_yields} and \Cref{fig:zcr_mll_postfit}
