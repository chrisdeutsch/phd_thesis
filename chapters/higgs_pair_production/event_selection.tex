...
\item[2017 data-taking period] Two DTTs based on different L1~seeds are used for
  ...
  \texttt{TAU20IM\_2TAU12IM\_4J12} L1~seed is used. This trigger requires
  ...
  the \FourJTwelve DTT. If the condition for the \FourJTwelve trigger is not
  fulfilled but the event has a central jet with $\pT > \SI{80}{\GeV}$ and a
  ...
  trigger is based on the L1 topological trigger system~\cite{TRIG-2019-02} that
  disambiguates jet and \tauhadvis ROIs and applies other topological
  requirements. At the L1 trigger, a jet with $\ET > \SI{25}{\GeV}$ that is not
  overlapping with a \tauhadvis ROI is required. Moreover, the \tauhadvis ROIs
  at the L1 trigger have to fulfil $\dRtautau < 2.8$. This trigger is referred
  to as the \LOneTopo DTT.
  ...
  by including the \FourJTwelve triggers is negligible.
