...
background processes and thus would not contribute to the signal
...
\begin{table}[htbp]
  ...
  \caption{Expected event yields in all three SRs prior to the fit. The yields
    ...
    SM~\HH event yields are given for the SM expectation. The \faketauhadvis
    background estimation technique employed in the \lephad channels does not
    distinguish between different sources of \faketauhadvis. The category
...
improvements in the \AccTimesEff by a factor of about 2 (1.5) for the \hadhad
channel (\lephad channels). This increase results from improved \tauhadvis
...
identification and $b$-tagging algorithms were significantly
improved~\cite{ATL-PHYS-PUB-2019-033,FTAG-2019-07}, yielding reduced mistag
rates at working points with tagging efficiencies similar to the previously used
working points. This allows to adopt looser object selection criteria while
maintaining background rates similar to the ones of the previous generation of
taggers. Second, Higgs boson pair production has distinct kinematic features
that can be used to distinguish between signal events and events containing jets
that are misidentified as $b$-jets or \tauhadvis, for example, using
multivariate methods. As a result, the best signal sensitivity is achieved with
a less stringent object selection.
...
The signal event yield after different steps of the SR event selection of the
\hadhad channel is shown in~\Cref{tab:cutflow}. Most events are rejected due to
the trigger selection, the \tauhadvis candidate selection, or the requirement of
two \btagged jets in the SR.

\begin{sidewaystable}[p]
  \centering

  \caption{Event yields after different selection steps in the \hadhad channel
    for the SM~\HH signals and four exemplary signals from decays of scalar
    resonances. The expected number of events are normalised using the cross
    sections predicted by the SM for the SM \HH production and using
