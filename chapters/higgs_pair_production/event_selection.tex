...
Selected by trigger and trigger requirements are fulfilled.

Exactly 2 \btagged jets.
1 $b$-tag regions have relatively low signal purities and are thus not

Opposite sign charge

Reject events where di-tau-mass reconstruction failed and \mMMC > 60
GeV. Reject low mass DY.

Lephad only: \mBB < 150 GeV for SR to define a top control region..


The acceptance times efficiency, \AccTimesEff, of the SM \HH signal in
the signal regions of all three channels is summarised
in~\Cref{tab:nonres_acc_times_eff} and compared to the \AccTimesEff of
...
\item[Improvements in \tauhadvis identification] The increased
  background rejection of the RNN \tauid compared to the BDT ID
  allowed to switch from the medium working point that was used
  ...
  algorithm based on a BDT discriminant, relative increase in tagging
