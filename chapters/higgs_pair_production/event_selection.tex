...
(in \Cref{sec:multivariate_analysis}). In addition, suitable control
...
Single tau trigger (STT), Di-\tauhadvis trigger (DTT)

Single lepton triggers (SLT), lepton-plus-\tauhadvis triggers (LTT)

An overview of the signal region event selection for all channels is
given in~\Cref{tab:event_selection}.

$H \to \bbbar$ candidate reconstruction

$H \to \tau\tau$ candidate reconstruction
...
\begin{table}[htbp]
  ...
  \caption{Overview of the signal region event selection in the \hadhad channel, and the \lephad SLT and LTT channels.

    Exact \pT thresholds vary with the triggers (the range is
    given). For taus in hadhad and jets thresholds are
    trigger-dependent. Forward jets are not used to select events
    (only to calculate MET). Requirements applied as part of the
    object selection are not repeated here.}%
