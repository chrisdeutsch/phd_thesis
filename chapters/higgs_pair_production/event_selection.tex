...
\begin{table}[h]
  ...
  \caption{List of single- and di-\tauhadvis triggers used to record
    data in the \hadhad channel.  For DTT, the leading tau / leading
    jet Triggers used for data taking in the \tauhad\tauhad channel.}%
...
\begin{sidewaystable}[p]
  ...
  \caption{List of single- and di-\tauhadvis triggers used in the
    ...
    collaboration are used and are summarised in the following. The
    ...
    refering to a requirement of $|\eta| < 2.3$ on jets, and
    \texttt{X} the number of jets fulfilling these conditions. Unless
    a trigger is based on the L1 topological (L1Topo) trigger
    system~\cite{TRIG-2019-02}, here chains using the
    \texttt{DR-TAU20ITAU12I-J25} seed, no disambiguation between
    objects at L1 is performed such that one \texttt{TAU20IM} will
    also ensure at least one count of \texttt{TAU12IM}, \texttt{J20},
    and \texttt{J12}. For chains based on L1Topo, disambiguation is
    performed and \tauhadvis at L1 are required to fulfil
    ...
    $\pT(\tau_0) > \SI{40}{\GeV}$ and $\pT(\tau_1) > \SI{30}{\GeV}$.}%
