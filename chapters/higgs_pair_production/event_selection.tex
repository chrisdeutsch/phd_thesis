...
defining the SRs are discussed in \Cref{sec:sr_and_cr_selection}.
...
Events passing the selections on the multiplicities of electrons, muons,
\tauhadvis, and jets (cf.~\Cref{tab:event_selection}) and the trigger selection
are used to define the SRs. Only regions with exactly two \btagged jets (2
$b$-tag regions) are considered as SRs. Regions with fewer \btagged jets are
dominated by background processes and thus would not contribute to the signal
sensitivity. Instead, 0 and 1 $b$-tag regions are used as control and validation
regions.
...
to define an orthogonal \ttbar CR depleted of signal events by inverting the
selection on \mBB. This region is used for measurements related to
\faketauhadvis background estimation.
...
backgrounds in which a quark- or gluon-initiated jet is reconstructed as a
