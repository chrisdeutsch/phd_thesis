...
$\bbbar\lephad$ or $\bbbar\hadhad$ final states. A loose selection is
applied to events that is largely determined by the trigger selection
adopted in the search. Distinguishing between signal and background
events based on object- and event-level observables is not the primary
goal of the event selection but rather of a multivariate analysis that
will be introduced at a later stage (in
\Cref{sec:multivariate_analysis}). In addition, suitable control and
validation regions are defined for the purpose of estimating
...
\begin{table}[htbp]
  ...
  \caption{Summary of the signal region event selections for the
    ...
    purposes. Requirements from the object selection introduced in
...
interest for the \hadhad channel. When an event fulfills both the STT
...
$|\eta| < 2.3$ instead. This change lead to a mismatch in the
pseudorapidity selection for \tauhadvis ($|\eta| < 2.5$) and jet ROIs
($|\eta| < 2.3$) at the L1 trigger, such that the L1 jet multiplicity
requirement was not specified as intended. This is problematic when
one or more \tauhadvis ROIs at L1 are reconstructed in the region
$2.3 \lesssim |\eta| \lesssim 2.5$ such that they are not
reconstructed as jet ROIs within $|\eta| < 2.3$ and thus not
contribute to the jet multiplicity. Moreover, the pseudorapdity
...
selection varies with the considered signal hypothesis. After a event
pre-selection,\footnote{Derivation skim, lepton veto, \tauhadvis
  pre-selection.} the efficiency of a signal event to be selected by
the trigger, the \tauhadvis candidates being trigger-matched and
passing their trigger-dependent \pT thresholds is about
\SI{40}{\percent} for SM \HH production. For the resonant production
of $pp \to X \to \HH$, the efficiency of this selection ranges from
\SI{30}{\percent} for $\mX = \SI{300}{\GeV}$ up to \SI{75}{\percent}
for $\mX = \SI{1600}{\GeV}$. The signal selection efficiency of
offline requirements on transverse momenta of jets and \dRtautau (for
\LOneTopo) range from \SI{80}{\percent} up to \SI{98}{\percent} for SM
