...
\item[\texttt{mediumRNN\_tracktwoMVA}] This chain started operation in period K
  ...
  RNN \tauid algorithm introduced in \Cref{sec:tauid}. The improved background
...
range from \SI{80}{\GeV} to \SI{160}{\GeV}.

The STT selection requires at least one \tauhadvis from the offline event
reconstruction to be geometrically matched ($\Delta R < 0.2$) to a \tauhadvis at
the HLT. Moreover, the \pT of the \tauhadvis has to exceed the \pT~threshold at
the HLT by \SIrange{15}{20}{\GeV}. The exact requirements are given
in~\Cref{tab:triggers_hadhad}. If an event fulfils both the STT and DTT
selection, precedence is given to the STT category.

\vspace{5em}

The \tauhadvis \pT requirements applied by the DTT selection remain unchanged
throughout Run~2 of the LHC. At the HLT, the \pT leading and \pT sub-leading
\tauhadvis candidate


At the HLT, the leading and sub-leading \tauhadvis
candidate has to have transverse momenta exceeding \SI{35}{\GeV} and
\SI{25}{\GeV}, respectively. In addition, two \tauhadvis candidates from the
offline event reconstruction are required to be trigger-matched
...
To ensure
that the triggers are close to fully efficient, two \tauhadvis from offline
reconstruction are required to be matched to \tauhadvis at the HLT and exceed
the corresponding HLT \pT-threshold by at least \SI{5}{\GeV}. The primary
limitations of DTTs arise at the L1 trigger, necessitating changes in the L1
seeds used for these triggers as the instantaneous luminosity of the LHC
increased during Run~2.
...
\begin{figure}[htbp]
  ...
  \includegraphics[width=0.9\textwidth]{selection/trigger_flowchart_clean}
  ...
  \caption{Flowchart of the \hadhad channel trigger selection. The leading and
    ...
    respectively.}%
  \label{fig:trigger_selection_flowchart_2}
\end{figure}
