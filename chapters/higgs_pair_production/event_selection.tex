...
(\texttt{DR-TAU20ITAU12I-J25}). Both trigger chains are used for this
search, which one is used depends on the phase space of the analysis.
If the transverse momentum of the sub-leading central jet from the
offline reconstruction exceeds \SI{45}{\GeV}, the trigger chain based
on the \texttt{TAU20IM\_2TAU12IM\_4J12} seed is used which requires to
additional jets with $\ET > \SI{12}{\GeV}$ at L1. The requirement on
the sub-leading central jet ensures that the L1 trigger seed is close
to its efficiency plateau. This type of trigger will be referred to as
\FourJTwelve hereafter. If the condition for the \FourJTwelve trigger
is not fulfilled, but the event has a leading central jet from offline
...
system~\cite{TRIG-2019-02} which allows

\LOneTopo \FourJTwelve
