...
$Z \to \mu^{+}\mu^{-}$ (inclusive in the jet flavour label), \Wjets,
diboson, and $\ttbar V$.
...
regions are depicted in~\Cref{tab:postfit_yields_smhh}.
...
\begin{table}[htbp]
  ...
  \caption{Expected event yields in the three signal regions after the
    background-only fit to the observed data. The category ``other
    backgrounds'' combines minor contributions from
    $Z \to \tautau + (bl,cl,ll)$, $Z \to e^{+}e^{-}$,
    $Z \to \mu^{+}\mu^{-}$, \Wjets, diboson and $\ttbar V$. The
    expected SM \HH signal yield is shown with a signal strength of 1
    and nuisance parameters determined by the fit. Nuisance parameters
    only affecting the SM \HH signal are at their pre-fit values.}
...
\begin{table}[htbp]
  ...
  \caption{Expected event yields in the two most signal-like bins of
    the BDT (\hadhad) and NN (\lephad) discriminants after the
    background-only fit to the observed data. The category ``other
    backgrounds'' combines minor contributions from
    $Z \to \tautau + (bl,cl,ll)$, $Z \to e^{+}e^{-}$,
    $Z \to \mu^{+}\mu^{-}$, \Wjets, diboson, and $\ttbar V$.}
...
% Top quark (SLT): 50(ttbar):50(stop)
...
quark backgrounds are more abundant\footnote{The targeted
...
\SI{50}{\percent} from single top production in the SLT channel.

% Large amount of ttbar in lephad (a lephad-like final state is about
% 7 times more likely in ttbar than a hadhad one - ignoring fakes)
%
% Signal yield: larger in hadhad
%
% Difficulties in lephad: Huge ttbar contribution
%                         More difficult mass reconstruction for MMC and mHH
...
\begin{figure}[htbp]
  ...
  \caption{Distributions of the BDT input variables in the \hadhad
    ...
    overlayed with a normalisation corresponding to 400
    times~$\xsecggfvbf^{\text{SM}}$.}
\end{figure}
