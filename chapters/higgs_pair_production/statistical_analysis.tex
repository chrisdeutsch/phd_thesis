...
MLE \Cref{tab:breakdown}

nuisance parameters and impact of nuisance parameters on the POI

\todo[inline]{Rankings here... \Cref{fig:nonres_np_rankings}. Need
  breakdown comparing impact of data statistics to say that this is
  the limiting source currently.}

\begin{table}[htbp]
  \centering

  \caption{Decomposition of the variance of $\hat{\mu}$ into
    categories of uncertainty sources. The fraction of the variance on
    $\hat{\mu}$ originating from a category is estimated using
    ...
    variance of $\hat{\mu}$ (all NPs are allowed to vary) and
    $\Delta\hat{\mu}^2_{\text{w/o cat}}$ the variance after fixing the
    nuisance parameters in a given category to their best-fit value.
    For the data statistical uncertainty it is determined directly by
    fixing all NPs to their best-fit value. The fractions of
    sub-categories do not necessarily sum to the parent category due
...
under the asymptotic approximation. The test is not significant at the
levels for discovery commonly used in HEP and thus the background-only
hypothesis cannot be rejected. The best-fit signal strength is
