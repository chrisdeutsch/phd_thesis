...
Global observables also need to be randomly generated for the
estimation of the global significance. This section will focus on the
generation of the global observables related to the Barlow-Beeston
method previously described in \Cref{sec:barlow_beeston} that is used
to include statistical uncertainties on the background rate
predictions in the likelihood function. Due to the statistical nature
of these uncertainties they cannot be neglected when estimating the
global significances, particularly since these are highly ranked in
the analysis.
...
similar considerations can be made as for observables discussed in the
previous section. The phase space selected by pairs of bins can
partially overlap, leading to dependencies between the background rate
predictions (from simulation and CR data) between both bins. The
generation of these observables is simplified, however, since
resampling methods can be used.

The sample of simulated and control region events can be resampled
using non-parametric Bootstrap
methods~\cite{10.1214/aos/1176344552,efron1994introduction} by
assigning random bootstrap weights $w_i^{\text{b}}$ to all events. The
bootstrap weights are drawn from $\pois(1)$ as an approximation to the
...
$m_{cb}$ (the global observables of the Barlow-Beeston method) using
\begin{align*}
