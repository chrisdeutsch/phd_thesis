...
While the analysis is mostly limited by the size of the recorded \pp-collision
...
(FWER)~\cite{lehmann2005testing}, can be controlled by setting the critical
threshold of the individual tests such that the FWER remains below an acceptable
level. In HEP this is conventionally done by defining the global $p$-value
\begin{align*}
...
(greater) than \num{2.87e-7} ($5$), which ensures that the FWER is at most
\num{2.87e-7}.
...
response in the signal injection tests (cf.~\Cref{fig:local_pvalues_injected}),
...
\item[Observables] Observables are random variables representing the number of
  ...
  pairs of observables are estimated using MC simulation, allowing to construct
  ...
  matrix estimated in the second step.
...
\item[Global observables (Barlow--Beeston method)] The global observables
  ...
  simulations and CR data used for background estimation. For every bin, the
  effective number of unweighted events is recalculated using the resampled
  datasets. The resulting values of the global observables are then used to
  define toy experiments for the global significance estimation.
...
\item[Global observables (other systematic uncertainties)] The global
  ...
  correlated for all hypothesis tests and are drawn from a Standard Normal
  distribution.
...
local significance is shown in~\Cref{fig:q0_samplingdist}.
...
Lastly, the toy experiments can be used to estimate the joint distribution of
the local significances for all hypothesis tests under the background-only
hypothesis to illustrate the dependencies between tests. For this purpose, a
signed definition of the local significance given by
\begin{align*}
  z_{\text{local}}^{\text{signed}} = \sgn(\hat{\sigma}) \sqrt{q_0}
\end{align*}
is used, where $\sgn(\hat{\sigma})$ is the sign of the best-fit cross
...
the background-only hypothesis. The correlation matrix between the signed local
significances of all 20 tests is determined using the toy experiments and is
shown in \Cref{fig:corr_sig}. The figure illustrates the increasing correlation
between tests at large \mX due to the reduced capacity of the PNNs to
distinguish between adjacent mass points.
...
thus omitted in subsequent discussions.
