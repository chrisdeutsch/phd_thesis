...
probed that simultaneously describes expected number of events in bins
of the MVA discriminants in the signal regions and \mll in the \ZHF
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lccccc}
    ...
    \midrule
    SM \HH & BDT & NN & NN & \mll \\
    $X \to \HH$ & PNN(\mX) & PNN(\mX) & PNN(\mX) & \mll \\
    \bottomrule
  \end{tabular}
\end{table}
...
Higgs boson branching ratios as predicted by the SM. The non-resonant
production of \HH is not considered as a background to the search for
resonant \HH production.

Additional nuisance parameters are introduced into the model. First,
two free normalisation factors are included that vary the
normalisation of the \ZHF and \ttbar backgrounds in all
regions. Second, for every independent source of systematic
uncertainty a nuisance parameter is introduced that is constrained by
an auxiliary measurement. In general, these parameters can act on both
the normalisation and shape of the distributions predicted by the
model. Finally, statistical uncertainties on background rate estimates
using finite samples of simulated or control region events are
implemented according to the simplified Barlow-Beeston method
introduced in \Cref{sec:barlow_beeston}. This method introduces one
nuisance parameter for every bin entering the model.

The parameters of the models are inferred by maximising the
corresponding likelihood function for the observed data or Asimov
dataset. Likelihood-ratio tests are performed to test hypothesised
values of the POI. Unless otherwise noted, asymptotic approximations
are used for the sampling distributions of the likelihood-ratio test
statistic.
