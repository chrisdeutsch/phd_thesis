...
\item[Observables] Observables are random variables representing the number of
  ...
  distribution of all observables,\footnote{This approach is motivated by
    ...
    copula~\cite{nelsen}. The copula solely describes the dependencies between
    the random variables.} where the Gaussian copula is defined by the
  correlation matrix estimated in the second step.

\item[Global observables (Barlow--Beeston method)] (i.e.~$m_{cb}$ in
