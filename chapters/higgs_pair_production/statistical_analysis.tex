...
dependence between $A$ and $B$ is given by the overlap in phase space
selected by both bins. The relevant phase space can be partitioned
into three parts with associated observables $X_i$:
\begin{itemize}
...
\end{itemize} \todo[inline]{Phase space is probably not the right word here...}
...
background estimate. Excerpts of the correlation matrix obtained in
...
matrix of dimension $144 \times 144$. Similar matrices are derived in
...
\hadhad channel. This correlation matrix has a few noteworthy
features:
\begin{itemize}
\item Off-diagonal elements of submatrices for fixed \mX of the PNN
  discriminant are zero due to the bins being disjoint by
  construction.

\item The most background-like bins (bins numbered 1) feature a large
  correlation of about \SI{90}{\percent} or larger. This is due to the
  overlap of events that are easily rejected by the PNN
  discriminants. An overlap in these bins is expected, independent of
  the targeted mass, due to the presence of events that are
  kinematically incompatible with the resonant production of SM Higgs
  boson pairs. An example for such events are the ones where both
  \mMMC and \mBB are far from the mass of the Higgs boson.

\item For the three PNN discriminants considered
  in~\Cref{fig:correlation_matrix_observables_medium}, the observables
  of the most signal-like bins are expected to have little
  correlation.
...
method needs to fulfil certain requirements. First, the model needs to
accurately reproduce the marginal distributions of the observables,
i.e.\ Poisson distributions with rates given by the background
prediction, such that the distribution of test statistics for
individual tests at fixed \mX can be reproduced. Second, it needs to
model the dependence structure between observables to account for the
overlap of phase space selected by the bins entering the statistical
interpretation.

A multivariate extension of the bivariate Poisson distribution that
was previously used to determine the correlation coefficients between
observables would fulfil these requirements, however, such a model
becomes increasingly intractable due to the large number of
observables considered, particularly due to the presence of negatively
weighted events used for the background estimate\footnote{In the
  absence of negatively weighted events, toy experiments could be
  generated using pseudo-data generated by drawing a random weight
  from $\pois(w_i)$ for every event with weight $w_i$ used for the
  background estimate. Subsequently, the set of pseudo-data can be
  used to determine the values of all observables. Conceptually, this
  approach has some parallels to resampling methods like the
  Bootstrap.}. Instead a model based on the mathematical framework
...
\begin{align*}
  H(x_1, \dots, x_n) = \mathcal{C}(F_1(x_1), \dots, F_n(x_n)) \,\text{.}
\end{align*}

For the task of generating toy experiments, the only missing piece
that remains is the functional form of the copula, since the marginal
distributions are assumed to be known. The Poisson distribution can be
approximated by a Normal distribution provided the expected rate is
not too small, suggesting that the copula describing the dependence
structure of a multivariate Normal distribution might be applicable as
an approximation. This copula, referred to as the Gaussian copula, can
be derived using the inversion method described in Ref.~\cite{nelsen}
and is given by
\begin{align*}
...
The correlation matrix $\myvec{R}$ is estimated using the correlation
matrices derived with the bivariate Poisson model previously shown
in~\Cref{fig:correlation_matrix_observables}. These modelling
assumptions are checked empirically using simulation and comparing
directly to the bivariate Poisson model described earlier.

In \Cref{fig:copula_model_check} the conditional mean and variance of
the bivariate Poisson model and a bivariate of . Three scenarios are
shown reflecting parameter settings similar to what occurs in the
analysis.

The conditional mean agrees very well with the bivariate Poisson
model. Some differences are observed in the conditional variances
which feature some small deviations in bins with low expected rates
and non-vanishing covariance.

Why is the conditional variance less important than the conditional
mean? Important is the mean? Variance averages out?

When is this a good approximation?
- No or full correlation
- Large expected rates and any correlation
- Ignoring higher order interactions / effects?
- Inspect dependencies by looking at the conditional mean / variance?
...
\begin{figure}[htbp]
  ...
  \begin{subfigure}{\textwidth}
    \centering
    \missingfigure{Large correlation bkg like}
    \subcaption{}
  \end{subfigure}

  \begin{subfigure}{\textwidth}
    \centering
    \missingfigure{Large correlation signal like}
    \subcaption{}
  \end{subfigure}

  \caption{The conditional mean and variance.}
  \label{fig:copula_model_check}
\end{figure}


Simplifying modelling assumption are made. Instead of describing all
higher-order interactions, the modelling is restricted to pair-wise
correlations between observables. Further, in the limit of large
number of observations, the Poisson distribution can be approximated
by a Normal distributions. Hence it would be intuitive to use the
multi-variate Normal distribution to describe the pair-wise dependence
between observables.


Idea: generate Poisson random variables for every bin, following the
expected rate encoded in the likelihood function. But also following
the correlation structure determined by these large correlation
matrices.\footnote{There is a simple solution for this in cases where
  there are no negatively weighted events. Namely by resampling: Draw
  Pois(weight) for every event. This is not feasible here since there
  is a considerable amount of negatively weighted events.}

This can be done by noting that any joint distribution can be
expressed by the marginal distributions and 'copula' (Sklar's
theorem).

The algorithm for generating toy observables
- Singular correlation matrix

Graphical explanation in the bivariate case is in Appendix\todo{Add to appendix}.
