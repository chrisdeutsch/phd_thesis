...
scalar resonance. The quoted significance of $3.0\sigma$ of the excess
observed at $\mX = \SI{1000}{\GeV}$ does not account for the effect of
...
considered for the statistical interpretation.

This formulation of the FWER is equivalent to the $p$-value of a
hypothesis test that employs the maximum local significance over a
family of tests as a test statistic and observing
$\Zmaxlocal = \zmaxlocal$, and is thus called the global $p$-value or
\pglobal for brevity. This global $p$-value is subsequently translated
into the global significance, analogously to the treatment of local
$p$-values and significances, according to
\begin{align*}
  z_{\text{global}} = \Phi^{-1}(1 - \pglobal) \,\text{,}
\end{align*}
where $\Phi^{-1}$ is the quantile function of the Standard Normal
distribution and \pglobal the global $p$-value.

In practice, the global significance can be determined using
simulation provided a generative model for $H_0$ is available. A
number of $N$ toy experiments can be drawn from $H_0$, subsequently
performing the set of hypothesis tests on all experiments and noting,
separately for every toy experiment, the maximum local significance
over all tests, \zmaxlocal. The global $p$-value is then estimated as
the fraction of toy experiments with \zmaxlocal exceeding the observed
value
\begin{align*}
...
\end{align*}
where $z_{\text{local, obs}}^{\text{max}}$ specifically refers to the
maximum local significance obtained from the tests using observed
data. Other methods~\cite{Gross:2010qma} exist to determine the global
significance but are not used here\todo{Note that this method is also
  affected by the main problem.}.



Can generate toy experiments (from the b-only model that doesn't
exist) and count the numbers of toy experiments where at least one
exceeds the threshold. Equivalently can consider the number of toys
where the maximum local significance exceeds $3\sigma$.

Difficulty (dependencies): earlier have seen (injection tests, MVA
chapter) that the PNN lacks specificity in distinguishing between
different signal hypotheses at high mass. This is reflected in an
overlap in events between the most signal-like bins of high \mX
discriminants. This effect has to be considered when calculating the
FWER.

Problem: The hypothesis tests are performed using separate likelihood
functions defined for the same set of observed data. As a result, the
generative model used to define the likelihood function(s) cannot be used
to generate toy experiments as it would neglect dependencies between
discriminants.

Another problem: Since we fit different hypotheses, the MLE of NPs are
generally slightly different for every fit. Since the models only
agree pre-fit, we go with this but set approximate normalisation
factors for \ttbar and \ZHF.

A substitute background-only model is constructed that allows to draw
toy experiments. Three distinct parts need to be considered:
\begin{itemize}
\item Generation of pseudo-data
\item Generation of auxiliarly
  \begin{itemize}
  \item Statistical precision of bkg.\ estimate (BB, $\gamma$)
  \item Other systematic uncertainties ($\alpha$)
  \end{itemize}
\end{itemize}
...
Determine the covariance / correlation between the observables of a
pair of bins entering the analysis from simulation / control region
data. Expect vanishing correlation between bins of the same PNN
distribution; generally non-vanishing between bins of different PNN
distributions.

Independent:
\begin{align*}
  X_{i} \sim \pois(\lambda_i) \qquad \text{for} \qquad  i = 1, 2, 3
\end{align*}
and
\begin{align*}
  X &= X_1 + X_3 \sim \pois(\lambda_1 + \lambda_3) \\
  Y &= X_1 + X_3 \sim \pois(\lambda_2 + \lambda_3)
\end{align*}

\begin{align*}
  \rho_{XY} = \frac{\cov(X, Y)}{\sqrt{\var(X)\var(Y)}}
  = \frac{\lambda_3}{\sqrt{(\lambda_1 + \lambda_3) \cdot (\lambda_2 + \lambda_3)}}
\end{align*}


\subsubsection{Global observables}
