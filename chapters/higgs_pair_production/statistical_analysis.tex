...
background-only model to the observed data in all analysis
channels. The equivalent figures for the \lephad SLT and LTT channels
are documented in~\Cref{sec:app_additional_figures}.
...
\begin{table}[htbp]
  ...
  \caption{Expected event yields per physics process in the \hadhad
    signal region for signal-like bins of the PNN discriminant after
    the background-only fit to observed data in all regions. The two
    most signal-like bins are shown for $\mX = \SI{300}{\GeV}$ and
    \SI{500}{\GeV}. $\dagger$: The large discrimination power between
    signal and backgrounds of the PNN for large \mX leads to a small
    number of bins for the statistical interpretation due to the
    choice of re-binning algorithm. Thus, only the most signal-like
    bin is shown for $\mX = \SI{1000}{\GeV}$ and \SI{1600}{\GeV}.}%
