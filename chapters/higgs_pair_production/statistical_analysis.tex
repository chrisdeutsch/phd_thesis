...
\begin{figure}[htbp]
  ...
  after the background-only fit.]{Distributions of the MVA discriminants used
    ...
    \hadhad~(c) channel after fitting the background-only model to observed data
    in all regions. The signal overlay is scaled to the expected upper limit on
    the signal strength of $3.9$ from the combination of all channels. The
...
\begin{table}[htbp]
  ...
  the MVA discriminants after the background-only fit.]{Event yields in the
    ...
    affecting the signal process, which are kept at their nominal values.}
