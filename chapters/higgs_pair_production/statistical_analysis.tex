...
overlap of phase space selected by the bins entering the
interpretation.
...
observables considered and particularly due to the presence of
negatively weighted events used for the background
estimate\footnote{In the absence of negatively weighted events, toy
  experiments could be generated using pseudo-data generated by
  drawing a random weight from $\pois(w_i)$ for every event with
  weight $w_i$ used for the background estimate. Subsequently, the set
  of pseudo-data can be used to determine the values of all
  observables. Conceptually, this approach would be similar to
  Bootstrap resampling.}. Instead a model based on the mathematical
framework provided by Sklar's theorem~\cite{Sklar1959FonctionsDR} is
adopted, allowing to factorise the marginal distributions from the
dependency structure of the observables.
...
between random variables\footnote{The copula is uniquely identifiable
  ...
  for discrete marginal distributions.}. The joint distribution can be
expressed as
\begin{align*}
...
A copula derived (using the inversion method~\cite{nelsen}) from the
dependency structure of the multivariate Normal distribution is given
by
\begin{align*}
  \mathcal{C}_{\myvec{R}}(u_1, \dots, u_d) = \Phi_{\myvec{R}}(\Phi^{-1}(u_1), \dots, \Phi^{-1}(u_d))
\end{align*}
where $\Phi_{\myvec{R}}$ is the CDF of the multivariate Normal
distribution with mean $\myvec{0}$ and covariance equal to the
correlation matrix $\myvec{R}$. Further, $\Phi^{-1}$ is the quantile
