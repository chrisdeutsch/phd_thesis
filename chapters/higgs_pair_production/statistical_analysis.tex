...
simulation provided a probabilistic model of $H_0$ is available. A
...
that also rely on being able to draw toy experiments from $H_0$.
...
The need for such a model was already indicated by the large width of
the significance response in the signal injection tests for large \mX
in~\Cref{fig:local_pvalues_injected}, which is caused by a partial
overlap of events in the most signal-like bins of PNN discriminants
for adjacent mass points. This overlap is expected to extend to
background events\todo{TODO} and thus will have an effect on the
estimated global significance which has to be quantified.
...
global significance estimation. It is built from the pre-fit
expectation except for the normalisation factors for \ttbar and \ZHF,
which are set to approximate post-fit values of 0.97 and 1.35
respectively.
