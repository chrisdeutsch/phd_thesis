...
A formal comparison of the background-only and signal-plus-background hypothesis
is performed. A LRT-based test for the discovery of a positive signal
(cf.~\Cref{sec:hypotesting}) is used for this comparison. This test yields an
observed $p$-value of \SI{27}{\percent} and thus the background-only hypothesis
cannot be rejected.
...
compatible with non-resonant \HH production predicted by the SM but also with
its absence.
...
\begin{sidewaysfigure}[p]
  ...
  % \todo[inline]{The uncertainties are obtained from the likelihood profile -
  % asymmetric.}
\end{sidewaysfigure}
