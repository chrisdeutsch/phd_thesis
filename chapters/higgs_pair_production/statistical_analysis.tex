...
The results of the unblinded search for non-resonant production of SM
\HH are presented in the following for the combination of the \hadhad
and \lephad channels. Additionally, results are presented prior to the
combination of the \hadhad and \lephad channels to illustrate the
signal sensitivity of different channels and the differences in impact
of systematic uncertainties between channels.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% ATLAS_norm_Zhf      1.4021e+00 +/-  1.11e-01 %
% ATLAS_norm_ttbar    9.7030e-01 +/-  3.76e-02 %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
The normalisation factors of \ttbar and \ZHF are estimated using a
likelihood fit of the background-only model to be \num{0.97 +- 0.04}
and \num{1.40 +- 0.11}, respectively, which are consistent with the
result obtained from fits of the control region only.
...
to data combining all signal and control regions.

\todo[inline]{Merging for illustration:
  Top-quark = ttbar and single top production,
  other = ???
  SM Higgs = Combination of all single Higgs boson backgrounds
  \faketauhadvis backgrounds in the \hadhad channel distinguished between multi-jet (MJ) and top pair production (\ttbar)
}
...
\todo[inline]{Post-fit yield table...}
...
\begin{table}[htbp]
  ...
  \caption{Expected event yields in the two most signal-like bins of
    the BDT (\hadhad) and NN (\lephad) after the background-only
    fit. The category ``Other'' combines multiple smaller backgrounds:
  ...
  %                            "ttW", "ttZ"}        %
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  \label{tab:postfit_signallike_smhh}
\end{table}
