...
by the background model are compared to data in the \hadhad,
...
\begin{table}[htbp]
  ...
  \begin{subtable}{\textwidth}
    \centering
    \subcaption{Expected event yields in the signal regions (post-fit)}%
  ...
  \begin{subtable}{\textwidth}
    ...
    \subcaption{Expected event yields in the two most signal-like bins
      of the BDT (\hadhad) and NN (\lephad) discriminants (post-fit)}%
...
individual channel.\todo{Mention uncertainties on \ZHF and SM Higgs?}
...
The background-only hypothesis is compared to the alternative
including the SM \HH signal with non-negative signal strength. The
likelihood ratio test comparing both hypotheses yields an observed
$p_0$-value of \SI{27.5}{\percent}
% (expected $p_0$-value of \SI{25.2}{\percent} from the $\mu = 1$ Asimov dataset)
under the asymptotic approximation. The test is not significant and
thus the background-only hypothesis cannot be rejected. The best-fit
signal strength is $\hat{\mu} = 0.9\,\numpmerr{+1.8}{-1.5}$ for the
combination of all channels\footnote{The best-fit signal strength of
  the fit combining \hadhad channel and CR is
  $\hat{\mu} = 0.7\,\numpmerr{+1.9}{-1.6}$ and for \lephad channels
  and CR, $\hat{\mu} = 1.9\,\numpmerr{+3.7}{-3.2}$.}.
...
a sub-leading role, explaining a third of the variance on \muhat,
largely dominated by uncertainties on the modelling of the SM
backgrounds.
...
The effect of uncertainties on \muhat is inspected on the level of
individual nuisance parameters in~\Cref{fig:nonres_np_rankings}. A
ranking of the 15 nuisance parameters with the largest impact on the
signal strength estimation is shown, including the MLE of the nuisance
parameter and its confidence interval from the unconditional fit to
observed data.

Observations:

- Highly ranked nuisance parameters tend to agree with their pre-fit values

- Constraints on highly ranked nuisance parameters primarily originate
from the lephad channels, for example ttbar parton shower
(decorrelated from hadhad) and rQCD (intended to be
conservative). There is also the MadGraph uncertainty in hadhad that
is sort of constrained (low stats two-sample comparison)

- Bkg. prediction statistical uncertainties rank highly due to MVA
strategy
...
\SI{36.1}{\per\femto\barn} integrated luminosity.\todo{Impact of data
  statistical uncertainties is about the same -> Speaks for
  improvements in CP and the analysis. Instrumental decreased in full
  Run~2. Huge improvements in calibration measurements.}
...
\begin{sidewaysfigure}[p]
  ...
  \caption{Ranking of the post-fit impact of nuisance parameters on
    ...
    normalisation factors (red markers). NP rankings and MLE are
  ...
  \todo[inline]{Technically the uncertainties are asymmetric (MINOS).}
\end{sidewaysfigure}




Upper limits are set on the SM \HH (ggF + VBF) signal strength and
cross section using the \CLs method~\cite{Read:2002hq} at
\SI{95}{\percent} confidence level (cf.\
\Cref{sec:hypothesis_testing}). In~\Cref{tab:limits_non_resonant} the
limits...
...
\begin{table}[htbp]
  ...
  \caption{Upper limits on the total cross section of Higgs pair
    ...
    The expected limits are determined under the assumption that
    $\mu = 0$.}%
