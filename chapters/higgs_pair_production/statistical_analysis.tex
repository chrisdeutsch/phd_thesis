...
of the global significance being close to $2\,\sigma$, which is
obtained from analytical calculations under the assumption that the 20
...
\begin{figure}[htbp]
  ...
  \caption{Estimate of the distribution of the maximum local
    ...
    orange.}%
  \label{fig:zmax_toys}
  \todo[inline]{Remove kernel smoothing or mention in caption.}
\end{figure}
...
The set of toy experiments for the global significance determination
can be used to estimate the joint distribution of the 20 test
statisics under the background-only hypothesis. For this purpose the
signed local significance defined
as~$z_{\text{local}}^{\text{signed}} = \sgn(\hat{\sigma}) \sqrt{q_0}$
(using the asymptotic approximation) is considered as the test
statistic, with $\sgn(\hat{\sigma})$ being the sign of the best-fit
cross-section. Local significances defined in this way are marginally
distributed according to a Standard Normal distribution under the
background-only hypothesis. In \Cref{fig:corr_sig} the correlation
matrix of the 20 test statistics is determined using the toy
experiments thus providing an illustration of the dependencies between
...
An interesting observation can be made by comparing the global
signficance result based on toys, i.e.\
$z_{\text{global}} = \num{2.01 +- 0.05}$, with the approximation
assuming 20 mutually independent tests which yields \num{2.06 +- 0.04}
(using a toy-based maximum local significance of \num{3.09 +-
  0.03}). The independence assumption is naively expected to be a
lower bound, however, both are compatible. This might seem
counterintuitive given~\Cref{fig:corr_sig}, particularly with large
correlations between the test statistics for large \mX. However, one
can convince themselves by approximating the joint distribution of
$Z_{\text{local}}$ for the 20 tests by a multivariate normal
distribution with a covariance matrix given by the figure. In this
case one can simulate many experiments and compare with the assumption
of independen tests. The difference of between both (estimated global
significances) is about 0.02. Thus the test seem to be ``independent
enough'' such that this approximation is reasonably valid.
