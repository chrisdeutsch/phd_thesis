...
(i.e.\ $z_{\text{local}} = \sqrt{q_0}$) is examined using a separate
set of toy experiments generated using standard toy generation
...
slightly for most tests, however. For example the observed excess in
...
significance of \num{3.09} which results from the observed $q_0$
and~\Cref{fig:q0_samplingdist_q0sig}. The following analysis of the
...
dependencies between hypothesis tests. The following steps performed
separately for every toy experiment:
\begin{enumerate}
...
\item The values of the $q_0$ test statistic are determined for all 20
  ...
  and global observables in the likelihood functions with values of
  the toy experiment.
...
\item The maximum of the local significances over all 20 tests is
  determined.

\end{enumerate}
These steps are performed for \num{10000} toy experiments yielding an
estimate of the distribution of the maximum local significance (over
all 20 tests) under the background-only hypothesis.

The results of the toy experiments are shown in~\Cref{fig:zmax_toys}
illustrating the effect of multiple testing leading to an expectation
of \Zmaxlocal of about \num{1.8} even in the absence of a real
...
Additional checks: Global toys are compared to local toys showing good
agreement.
...
\begin{figure}[htbp]
  ...
  \todo[inline]{Note that this uses
    $z_{\text{local}} = \mathrm{sign}(\hat{\mu}) \sqrt{q_0}$.}
\end{figure}
