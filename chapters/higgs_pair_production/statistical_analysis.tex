...
generation of the toy experiments from the background-only hypothesis. While the
...
a description of a single discriminant at a time. However, a background model
that jointly describes all binned discriminants is required to generate toy
experiments for the global significance estimation. The need for such a model
was already indicated by the large width of the significance response in the
signal injection tests (cf.~\Cref{fig:local_pvalues_injected}), which is caused
by a partial overlap of events selected by the most signal-like bins of the PNN
discriminants for adjacent mass points.

A substitute background model is constructed that accounts for dependencies
...
parts are highlighted hereafter.

The model is constructed assuming the pre-fit expectation for all backgrounds
except for the \ttbar and \ZHF normalisation factors, which are set to
approximate post-fit values of 0.97 and 1.35, respectively. The model has three
distinct parts, each describing a certain kind of random variable:
\begin{description}

\item[Observables] refer to the observed number of events in a given bin of a

  , correspond $n_{cb}$ in \Cref{eq:likelihood_histfactory},
  % Poisson marginals connected by Gaussian copula

  The distribution of observables is modelled using a joint distribution
  described by Poisson marginal distributions which are Gaussian copula

  Correlation matrix describing the Gaussian copula is derived from MC
  simulation.

\item[Global observables (Barlow--Beeston method)] ($m_{cb}$ in
  \Cref{eq:likelihood_histfactory_constraints})
  % Resampling of the MC and then determining the associated observables for
  % every bin

\item[Global observables (other systematic uncertainties)] ($a_p$ in
  \Cref{eq:likelihood_histfactory_constraints})
  % Fully correlated drawn from Standard Normal distribution.
