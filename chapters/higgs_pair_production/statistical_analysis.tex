...
acceptable level. In HEP this is typically done by defining the global $p$-value
\begin{align*}
...
over all tests, \zmaxlocalobs is the observed value of \Zmaxlocal, and $H_0$
refers to the background-only hypothesis. The global significance is defined as
\begin{align*}
...
to determine the global significance. In general, the distribution of \Zmaxlocal
is unknown and needs to be determined by simulation. In practice, this is done
as follows: First, a toy experiment is drawn from the background-only
model. Second, the set of hypothesis tests is performed for a given toy
experiment and the maximum local significance over all tests is
determined. These steps are repeated $N$ times yielding a sample of random
variates for \Zmaxlocal under the background-only hypothesis. The global
$p$-value is then estimated according to
\begin{align*}
