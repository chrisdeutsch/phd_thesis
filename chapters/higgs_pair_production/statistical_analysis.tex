...
The backgrounds relevant to the signal extraction change with
increasing \mX. For the search at low mass, the dominant backgrounds
are top quark production and backgrounds with jets being misidentified
as \tauhadvis. These background processes become less relevant as
larger \mX are considered at which point the production of \Zjets
...
in~\Cref{fig:resonant_mva_postfit}. In the most signal-like bin of the
...
The background-only model is compared with the signal-plus-background
hypothesis using a likelihood ratio test for all considered signal
hypotheses. The combination of all analysis channels yields an excess
at $\mX = \SI{1000}{\GeV}$ with a $p$-value of $\num{1.3e-3}$ (under
the asymptotic approximation) and is therefore significant at a level
of $\num{3.0}\sigma$\footnote{By convention in HEP the discovery
  significance in is expressed in terms of $\sigma$ given by
...
$\num{2.8}\sigma$ when performing the test only including the \hadhad
signal region and the \ZHF control region. The $p$-values for all
considered signal hypotheses are shown in \Cref{fig:local_pvalues}
separately for the \lephad and \hadhad channels and their
combination. All significances quoted thus far do not account for
multiple hypothesis testing and are therefore referred to as local
...
being significant at the $2\sigma$ level. At these mass scales a broad
significance response is expected for a true signal due to the
degradation in the absolute resolution of the $HH$ system mass and the
lack of specificity of the PNN discriminants in distinguishing between
different signal hypothesis as previously discussed in
\Cref{sec:mva_pnn}. This is further illustrated in
\Cref{fig:local_pvalues_injected} where the observed local
significances are compared to the expectation with an injected signal
at the best-fit signal strength. The width of significance response of
the observed excess is similar to the expectation obtained from the
injection tests.
...
\begin{figure}[htbp]
  ...
  \caption{Expected local $p$-values from Asimov datasets after
    injecting signals with resonance masses of 900, 1000, and
    \SI[group-minimum-digits=5]{1100}{\GeV} at their best-fit signal
    strength from the unconditional fit to observed data. The
    respective signal is injected in all signal regions and the
    expected $p$-value resulting from the statistical analysis of the
    combination of all channels is shown.}
  \label{fig:local_pvalues_injected}
\end{figure}

The best-fit cross section of $pp \to X(\SI{1000}{\GeV}) \to HH$
is~\SIpmerr{17.4}{+7.5}{-6.5}{\femto\barn} obtained from the fit of
the PNN discriminants for $\mX = \SI{1000}{\GeV}$ to observed data in
all channels.

$4b$: \SI{6.5}{\femto\barn} (exp.\ \SIpmerr{8.15}{+1.17}{-0.77}{\femto\barn})

Upper limits are set on $\sigma(pp \to X \to \HH)$ as a function of
\mX using the \CLs method. The observed and expected limits are shown
in \Cref{fig:res_upper_limits}.
...
\begin{figure}[htbp]
  ...
  \caption{Upper limits on $\sigma(pp \to X \to \HH)$ as a function of
    the mass of the scalar resonance using the \CLs method at
