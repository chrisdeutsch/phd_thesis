...
\subsubsection{Inter- / Extrapolation}

\todo[inline]{Uncertainties split in norm and shape part?}
\todo[inline]{Shape uncertainties seem to be polynomial interpolation and linear extrapolation}

Uncertainties are provided in the form of templates corresponding to
$\pm 1\sigma$ variations. The likelihood model needs to interpolate
and extrapolate the effect of variations for any value of the
associated nuisance parameter and not just $\pm 1$ or 0.

The inter- and extrapolation is performed bin-wise using a piecewise
definition using exponentials for extrapolation and a polynomial for
interpolation~\cite{cranmer2012}. The function and its first and
second derivative are required to be continiuous in the value of the
nuisance parameter.
