...
channels. Additionally, results are presented prior to the combination
of the \hadhad and \lephad channels to illustrate the signal
sensitivity of the individual channels and the differences in impact
of systematic uncertainties.

All results are obtained from the model using the signal strength as
the POI unless otherwise noted. Further merging of physics processes
is performed for the illustration only. The contributions from top
quark pair and single top production are combined into the ``top
...
obtained from fits of the control region only.
...
combining all signal and control regions. After the fit the
background-model provides a good description of the observed data in
the signal regions. The post-fit event yields corresponding to these
regions are summarised in~\Cref{tab:postfit_yields_smhh}.
...
\begin{table}[htbp]
  ...
  \begin{subtable}{\textwidth}
    ...
    \subcaption{Inclusive event yields in the signal regions}
  ...
  \begin{subtable}{\textwidth}
    ...
    \subcaption{Event yields in the two most signal-like bins of the
      BDT (\hadhad) and NN (\lephad) discriminants}
    \label{tab:postfit_yields_smhh_signallike}
  \end{subtable}

  \caption{Expected event yields in the three signal regions (a) and
    in the two most signal-like bins of the MVA discriminants (b)
    after the background-only fit to the observed data in all
    regions. The category ``other backgrounds'' combines minor
    contributions from $Z \to \tautau + (bl,cl,ll)$,
    $Z \to e^{+}e^{-}$, $Z \to \mu^{+}\mu^{-}$, \Wjets, diboson and
    $\ttbar V$. The expected SM \HH signal yield is shown with a
    signal strength of 1 and nuisance parameters determined by the
    fit. Nuisance parameters only affecting the SM \HH signal are at
    their pre-fit values.}
...
The choice of an analysis strategy based on using multivariate
discriminants to extract the signal of interest from loosely
preselected events limits the conclusions that can be drawn regarding
the backgrounds that are relevant to the signal extraction
process. Therefore, in \Cref{tab:postfit_yields_smhh_signallike} the
expected event yields are shown for the two most signal-like bins of
the MVA discriminants to illustrate the signal sensitivity and
background composition in a phase space most similar to the signal
process.
...
expectation of about 8 events. The top quark, single Higgs boson, and
\faketauhadvis backgrounds contribute with approximately 4 expected
events each. The signal selection efficiency of the two most
signal-like bins with respect to the pre-selection is close to
\SI{50}{\percent} yielding the largest signal-to-background ratio of
any individual channel.\todo{Mention uncertainties on \ZHF and SM Higgs?}
...
top quark and \ZHF processes. Compared to the \hadhad channel, top
quark backgrounds have higher abundance\footnote{The targeted
  ...
  $W \to \ell \nu_{\ell}$ ($\ell = e$ or $\mu$) are considered as the
...
\SI{50}{\percent} from single top production in the SLT channel. The
signal selection efficiency is considerably smaller than that of the
\hadhad channels which results from the more abundant top backgrounds
(combined with the re-binning) and a worse $H \to \tautau$ mass
reconstruction performance in the \lephad channel.

The modelling of the MVA input variables in the \hadhad channel can be
further inspected in~\Cref{fig:postfit_mva_inputs}, which shows the
variables after the background-only fit of the MVA discriminants
combining all channels of the analysis. The post-fit background model
describes the observed data well with some minor discrepancies in the
...
A post hoc analysis\todo{Put in appendix} of the feature observed at
low \dRtautau is performed by inspecting the dependency of the
measured \ZHF normalisation factor as a function of the transverse
momentum of the $Z$ boson by performing a simplified normalisation
measurement in the \ZHF control region in bins of~$\pT(Z)$
reconstructed from the lepton transverse momenta. This cross-check
shows a trend of decreasing \ZHF normalisation factors with increasing
$\pT(Z)$ which can lead to differences of about
\SIrange{5}{10}{\percent} at typical values of $\pT(Z)$ after the
\hadhad pre-selection ($\SI{100}{\GeV} < \pT(Z) < \SI{200}{\GeV}$)
when comparing to the $\pT(Z)$-inclusive normalisation factor. Due to
the anticorrelation between $\pT(Z)$ and \dRtautau, this effect will
be further enhanced in regions of low \dRtautau. The acceptance
uncertainties on the \ZHF background, which are derived based on
simulation, might not fully account for this effect that is observed
in control region data. In the future it could be beneficial to
control for such differential effects when measuring the \ZHF
background in the control region.
