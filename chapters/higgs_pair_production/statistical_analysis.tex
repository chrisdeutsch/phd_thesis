...
becomes intractable due to the large number of observables considered,
and in particular due to the presence of negatively weighted events
used for the background estimate\footnote{In the absence of negatively
  weighted events, toy experiments could be generated using
  pseudo-data generated by drawing a random weight from $\pois(w_i)$
  for every event with weight $w_i$ used for the background
...
The parameters $\myvec{R}$ of the Gaussian copula is estimated using
...
subtle differences between both models.
\begin{itemize}

\item The conditional mean of $B$ given $A = a$ between both models
  agrees well even when considering bin pairs with the lowest rates
  $\lambda_i$ relevant for the analysis.

\item The conditional variance of $B$ given $A = a$ illustrates that
  both models are not equivalent. For bin pairs with small expected
  rates $\lambda_i$, the variance of the distribution of $B$ for fixed
  $A = a$ differs by up \SI{15}{\percent} to between both models.

  Pair-wise linear correlation

\item In the case of disjoint or fully\todo{Mathematically this is a
    bit funky since the Gaussian PDF is not defined anymore.}
  overlapping bins (not shown in the figure) the bivariate Poisson
...
\footnote{The matrix $\myvec{R}$ is singular due to multicollinearity
  between the observables. This is due to 19 implicit constraints from
  the requirement that the sum of observables is the same for all 20
  discriminants resulting in 19 singular eigenvalues for
  $\myvec{R}$. These are removed using singular value
  decomposition(???) prior to the generation of random variates.}
