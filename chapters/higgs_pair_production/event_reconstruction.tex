...
in~\Cref{tab:overlap_removal} following the recommendations developed
by the ATLAS collaboration. The overlap removal between jets and
\tauhadvis
...
\begin{table}[htbp]
  ...
  \caption{Summary of the sequential overlap removal algorithm with
    ...
    top to bottom, rejecting objects in the ``Reject'' column in
    favour of objects in the ``Accept'' column if the condition is
    fulfilled, until all ambiguities are resolved.}%
