...
measurements.\todo{\ttbar normalisation?}

Pulls and constraints of nuisance parameters are examined for all
measurements\todo{Appendix?}. The measured nuisance parameters agree
...
precision\todo{fill}. The \tauhadvis energy scale measurements are
performed in $Z \ra \tau_{\mu} \tauhad$ tag-and-probe\todo{citation?}
...
\Cref{fig:ttbarSF_postfit_ptmtw} shows the inclusive SF-CR after the
maximum likelihood fit\todo{interpretation}.
...
% Distribution of \tauhadvis \pT and $\eta$ in the scale
%     factor control region (SF-CR) prior to the maximum likelihood
%     fit. Events with \tauhadvis \pT larger than \SI{200}{\GeV} are
%     merged into the last bin. The uncertainty contains all statistical
%     and systematic uncertainties except for normalisation
