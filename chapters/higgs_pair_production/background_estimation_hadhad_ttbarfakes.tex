...
\todo[inline]{Talk about some motivation for this method: Absence of a
  good ttbar control region that is close to hadhad in the final state
  -- need to go to lephad for measurement.}
...
discussed in~\Cref{sec:hadhad_multijet} where both \tauhadvis
...
is significantly larger for quark-initiated jets\todo{Numbers?}.
...
% Two approaches were investigated, one being the direct
% measurement of the misidentification efficiency in data (the so-called
% \textit{fake rate})
% \begin{align*}
%   \varepsilon_\text{mis-ID}^\text{data} = \frac{N_\text{post-ID}^\text{data}}{N_\text{total}^\text{data}}
% \end{align*}
...
%   \frac{N_\text{post-ID}^\text{data} / N_\text{total}^\text{data}}{N_\text{post-ID}^\text{MC} / N_\text{total}^\text{MC}}
%   = \frac{N_\text{post-ID}^\text{data}}{N_\text{post-ID}^\text{MC}} \cdot \frac{N_\text{total}^\text{MC}}{N_\text{total}^\text{data}}
% \end{align*}
...
A top control region (SF-CR) is defined in the $\Plepton + \tauhadvis$
reconstructed final state with \Plepton being either an electron or a
muon. The region is similar to the top control region used in the
\lephad channel where it is used to calculate fake factors for
\tauhadvis mimicked by jets produced in \ttbar (c.f.\
\cref{sec:}). Minor alterations are applied to the region definition
to ensure a consistent \tauhadvis selection with the \hadhad SR.

The region is defined by requiring exactly one \tauhadvis passing
loose identification, exactly one electron or muon passing isolation
and identification criteria\todo{reference}, and exactly two \btagged
jets. Only events passing single lepton trigger selection are
considered. The \tauhadvis selection is adapted to more closely follow
the selection of the \hadhad channel by requiring \tauhadvis
candidates to fulfil $\pT > \SI{25}{\GeV}$ and $|\eta| < 2.5$ (instead
of $\pT > \SI{20}{\GeV}$ and $|\eta| < \num{2.3}$ in the \lephad
channel).
...
Orthogonality between the SF-CR and the signal region of the \lephad
channel is ensured by requiring $\mBB > \SI{150}{\GeV}$.
...
\SI{66}{\percent}, is \ttbar with a final state containing
$\ell + \tau$ where $\ell$ is either an electron or muon and the
\taulepton decays hadronically. This contribution will be subsequently
referred to as di-leptonic \ttbar. The production of \ttbar with
\tauhadvis originating from quark or gluon jets, which is the process
of interest for this measurement, constitutes a purity
of~\SI{28}{\percent}.  Minor backgrounds in this region are single top
(\SI{4}{\percent}) and vector boson production in association with
jets (\SI{2}{\percent}). The contribution of \multijet background is
...
\begin{figure}[htbp]
  ...
  \includegraphics[width=0.49\textwidth]{ttbarSF/prefit_sfcr/MTWVR}

  \caption{Distribution of \tauhadvis \pT and \mTW in the scale factor
    control region (SF-CR) prior to the maximum likelihood fit. Events
    with \tauhadvis \pT (\mTW) larger than \SI{200}{\GeV} are merged
    into the last bin. The uncertainty contains all statistical and
    systematic uncertainties except for normalisation uncertainties on
    \ttbar arising from its modelling in simulation. Modelling
    uncertainties affecting the shape of the distributions are
    included.}
...
\todo[inline]{Plots of \faketauhadvis \pT at least.}
...
the reconstructed \tauhadvis candidate (c.f.\ \Cref{sec:tauid}). This
...
states is shown in~\Cref{fig:ttbarsf_mtw_pdf} for the considered
control region.
...
\begin{figure}[htbp]
  ...
  \todo[inline]{Remove ATLAS label, remove / 40 GeV.}
\end{figure}
...
is also expected to roughly follow a descending order\todo{Maybe
...
uncertainty over a range of \tauhadvis \pT. \todo{Interpretation. What
  if pT > 150?}
...
approach. In~\Cref{tab:ttbarSF_yields} the event yield of \ttbarFakes
...
% Measurement uncertainties on total yield are highly correlated: ca. 90%
% PS uncertainty for 1-prong taus is anti-correlated due to sign-change of uncertainty
...
previously described in \Cref{sec:hadhad_multijet}, necessitates a
...
of \tauhadvis at the HLT are imposed.\todo{Lower cut prohibits
  relating ID-region to anti-ID region SFs.}
...
region with high purity of \ttbarFakes (\todo{number}). Matching
offline reconstructed \tauhadvis to HLT \tauhadvis reduces the total
yield (\todo{number}) and purity (\todo{number}) of \ttbarFakes in the
\antiid region.
...
in~\Cref{fig:ttbarSF_antiid_SF}. The efficiency of selecting a quark-
