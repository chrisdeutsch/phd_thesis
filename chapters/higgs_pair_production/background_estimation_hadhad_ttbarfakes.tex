...
possible bias in the estimated SFs due to \tauhadvis-trigger turn-on effects is
...
due to differences in the trigger efficiency for both triggers. Generally, this
...
\tauhadvis candidates with $\pT > \SI{40}{\GeV}$ are considered, which
corresponds to the threshold applied to \tauhadvis candidates passing the
\texttt{tau35} trigger during offline event reconstruction. After this
...
\texttt{tau35} triggers. This difference in \faketauhadvis selected by the
\texttt{tau25} and \texttt{tau35} trigger is a potential source of bias in the
