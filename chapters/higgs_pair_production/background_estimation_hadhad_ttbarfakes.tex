...
\begin{table}[htbp]
  ...
  \caption{Multiplicative event weights for the data-driven correction of \ttbar
    with \faketauhadvis from simulation. Events are distinguished by whether the
    leading \tauhadvis candidate ($\tau_{\text{lead}}$), the sub-leading
    \tauhadvis candidate ($\tau_{\text{subl}}$), or both are faked by a quark-
    or gluon-jet. SFs for \faketauhadvis without trigger requirements are
    denoted as $\text{SF}_{\text{loose}}$; SFs with isolation and identification
    requirements at trigger-level by $\text{SF}_\text{loose+trig.}$.}%
