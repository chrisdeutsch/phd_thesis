Top quark pair production with at least one \tauhadvis candidate originating
...
(cf.~\Cref{sec:hadhad_trigger_selection}) which are applied in addition to
offline \tauid.
...
\item \verb|HLT_tau25_medium1_tracktwoEF| \emph{or}
  \verb|HLT_tau25_mediumRNN_tracktwoMVA| %(\SI{37}{\ifb})
\end{itemize}
...
unprescaled single-lepton triggers. This allows to re-run the single-\tauhadvis
triggers used in this measurement during offline event reconstruction without
application of a prescale. Only the \verb|HLT_tau25_medium1_tracktwo| trigger
...
in~\Cref{fig:ttbarsf_region_summary_prefit}.\todo{Remove ``ATLAS Internal''
  label.}
...
A discriminant is required to distinguish contributions of \ttbar with and
...
\Ntracks. The fit model is constructed from \mTW distributions predicted using
simulated \ttbar, single top, and \Vjets events. The sample of \ttbar events is
split by whether the reconstructed \tauhadvis candidate is a \faketauhadvis or
not. The overall normalisation of the \ttbar background, irrespective of whether
events contain a \faketauhadvis, is allowed to vary freely in the model.  In
every region of \tauhadvis candidate \Ntracks and \pT, an unconstrained SF is
introduced that changes the normalisation of the \ttbarFakes background in this
region. These \faketauhadvis SFs are considered the parameters of interest of
the measurement.
...
same in all \tauhadvis candidate \pT and \Ntracks regions.
...
\begin{figure}[htbp]
  ...
  \caption{Histograms of the \mTW distributions in two exemplary \tauhadvis
    candidate \Ntracks and \pT regions. Two exemplary \mTW distributions as
    they enter the likelihood fit determining the \faketauhadvis scale
    factors. Only SF-CR events passing the
    \texttt{HLT\_tau25\_medium1\_tracktwo} trigger and \tauhadvis
    trigger-matching are considered. Events with $\mTW > \SI{200}{\GeV}$ are
    merged into the last bin of the histogram.}%
...
measuring the \faketauhadvis SFs. Generally, these uncertainties can affect the
normalisation and shape of the expected \mTW distribution for a given process in
all regions entering the fit. The uncertainties included in the SF measurement
and their treatment closely follows the approach taken in
\Cref{sec:uncertainties,sec:statistical_analysis} to interpret the results of
the search for Higgs boson pair production. The main sources of uncertainty in
the SF measurement are therefore only briefly summarised.

Experimental uncertainties affecting the reconstruction and selection
efficiencies of electrons, muons, \tauhadvis, and jets are considered in the SF
measurement, including uncertainties on the efficiencies and mis-tag rates of
...
triggers.\footnote{Uncertainties on single-\tauhadvis trigger efficiencies are
  only considered in SF measurements for events after trigger-level \tauid.}


% PRW & LUMI

Statistical uncertainties from the finite size of the simulation samples used to
construct the background templates are considered and implemented in the
likelihood fit using a simplified Barlow-Beeston
...
% Major detector-related uncertainties
% Statistical uncertainties of the finite size of the simulated samples

% Theory uncertainties on ttbar
% - Modelling of the hard scattering interaction and the matching of NLO matrix element generator with parton shower
% - Variation of the parton shower, hadronisation model, and underlying event modelling by using alternatiev parton shower generator
% - Missing higher order in the perturbative expansion by performing scale variations
% - The modelling of additional radiation

% Theory uncertainties on minor backgrounds
...
Top modelling uncertainties: \Cref{app:top_uncertainties}.
...
\begin{figure}[htbp]
  ...
  \caption{Post-fit distribution of \tauhadvis \pT and \mTW in the SF-CR after
    requiring the \tauhadvis to be matched to the
    \texttt{HLT\_tau25\_medium1\_tracktwo} trigger. Events with \tauhadvis \pT
    ...
    uncertainties, including post-fit uncertainties on \faketauhadvis scale
    factors and the \ttbar normalisation factor.}
...
according to the selection strategy described in~\Cref{sec:trigger} and
