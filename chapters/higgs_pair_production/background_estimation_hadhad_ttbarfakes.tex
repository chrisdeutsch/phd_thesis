...
the finite size of the simulation samples used to construct the fit model are
included according to the method by Barlow and
Beeston~\cite{barlow1993,conway2011} introduced in \Cref{sec:barlow_beeston}.
...
collaboration, which are documented in \Cref{app:top_uncertainties}. An
...
correlated in the fit model if they originate from the same source. Effects of
the \ttbar modelling uncertainties on the shape of the \mTW discriminants and
the expected number of events in the different \tauhadvis candidate \Ntracks and
\pT regions are implemented in the fit model.
...
quarks, a normalisation uncertainty of \SI{30}{\percent} is applied to the minor
\Vjets background.

% A reduced set of theory uncertainties for minor backgrounds is considered. For
% the production of single top-quarks, the uncertainties on the cross-section are
% included in the fit model. Due to the known normalisation discrepancy in \Vjets
% in the presence of jets originating from heavy flavour, an uncertainty of
% \SI{30}{\percent} is applied to the overall normalisation of the process.
