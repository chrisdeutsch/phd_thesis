...
are affected by this uncertainty. \Cref{tab:ttbarSF_yields} summarises the
...
predictions agree within uncertainties with the exception of the
FT~component. The predicted number of FT~events is smaller after application of
\ttbarFake SFs due to SFs being smaller than unity for \faketauhadvis with
high~\pT.
...
\tauhadvis are required to fail the loose working point but exceed a RNN \tauid
...
The SFs measured in the Anti-ID region are shown
...
Anti-ID region of the \hadhad channel. The rules for applying SFs remain
unchanged with respect to the SF application in the ID-region. The context and
impact of the SFs for the Anti-ID region on the \multijet estimate will be
...
\begin{figure}[htbp]
  ...
  \caption{SFs for \faketauhadvis from \ttbar simulation in the Anti-ID region
    separated \tauhadvis identification criteria applied at
    trigger-level. Figures (a) and (b) compare the SFs with and without \tauid
    at the HLT. The effect of different \tauhadvis trigger chains on the
    extracted SFs is shown in (c) and (d). The last bin summarises events with
...
\todo[inline]{Plots here or appendix: \ttbarFakes estimate in the \hadhad SR
  (taupt?). taupt in the Anti-ID SF-CR before and after trigger matching?}
