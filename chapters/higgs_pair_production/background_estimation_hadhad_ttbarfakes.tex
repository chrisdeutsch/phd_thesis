...
data-driven corrections in the form of \faketauhadvisC scale factors (SFs)
...
\faketauhadvisC backgrounds from \ttbar and multi-jet are highlighted that
...
\item The probability that a quark- or gluon-initiated jet reconstructed as a
  ...
  have larger fake rates than gluon-initiated jets. As a result, \faketauhadvisC
...
\item In the \hadhad channel, no suitable \ttbarFakes CR can be defined that
  separates \faketauhadvisC backgrounds from \ttbar and multi-jet while
...
\end{itemize}
The main disadvantage of separately estimating the \faketauhadvisC background
from \ttbar and multi-jet is the inflation of systematic uncertainties on the
estimate of the total \faketauhadvisC background compared to a combined
...
them from \faketauhadvisC backgrounds. Second, the search is limited by
...
\faketauhadvisC background estimation. Minor changes are applied to its
...
% the SF-CR. In \ttbar the \faketauhadvisC candidate is more likely to
...
The \faketauhadvisC SFs are measured using a simultaneous likelihood fit of the
binned \mTW distributions in all regions of \tauhadvis candidate \pT and
\Ntracks. The fit model is constructed using simulated \ttbar, single-top-quark, and
...
normalisation of the \ttbarFakes background in this region. These \faketauhadvisC
...
SF measurement. Uncertainties on the cross sections of single-top-quark and \Vjets
...
The measured \faketauhadvisC SFs are shown in \Cref{fig:ttbarSF_postfit_SF}.
...
\begin{figure}[htbp]
  ...
  \caption[\FaketauhadvisC SFs for different \tauid criteria.]{\FaketauhadvisC SFs
...
\begin{figure}[htbp]
  ...
  SF-CR.]{Post-fit distributions of \tauhadvis candidate \pT and \mTW in the
    ...
    including post-fit uncertainties on \faketauhadvisC SFs and on the overall
...
\faketauhadvisC SFs and the \ttbar normalisation factor. Due to this coupling,
...
% partially compensated by an increase in the \faketauhadvisC scale factors. This
% coupling between \faketauhadvisC scale factors and \ttbar normalisation
...
\begin{figure}[htbp]
  ...
  \faketauhadvisC SF measurement for the \texttt{HLT\_tau25\_medium1\_tracktwo}
...
\begin{figure}[htbp]
  ...
  \caption[Systematic variations of the \faketauhadvisC SFs for the
  \texttt{HLT\_tau25\_medium1\_tracktwo} trigger.]{\FaketauhadvisC SF variations
...
\subsubsection{Application of \FaketauhadvisC Scale Factors in the \hadhad
  Channel}

The \ttbarFakes background in the SR of the \hadhad channel is estimated by
applying \faketauhadvisC SFs to \ttbar events from simulation with at least one
...
thresholds at the HLT~(L1 trigger) on the \faketauhadvisC SFs is estimated. The
...
% scale factors for high \pT \faketauhadvisC tend to be smaller than unity. This
...
\begin{table}[htbp]
  ...
  without application of the \faketauhadvisC~SFs.]{Expected number of \ttbarFakes
    events in the \hadhad SR with (right) and without (left) application of the
    \faketauhadvisC~SFs. Only MC statistical uncertainties are shown for the
    estimate without application of the \faketauhadvisC~SFs. The background
    estimate using the \faketauhadvisC~SFs includes statistical uncertainties and
...
% uncertainty for 1-prong \faketauhadvisC candidates, making up $\frac{3}{4}$ of
...
\subsubsection{Estimation of \FaketauhadvisC Scale Factors for Anti-\tauhadvis}
...
\begin{figure}[htbp]
  ...
  \caption[\FaketauhadvisC SFs for anti-\tauhadvis and different \tauid criteria
  applied at trigger-level.]{\FaketauhadvisC SFs for anti-\tauhadvis and
