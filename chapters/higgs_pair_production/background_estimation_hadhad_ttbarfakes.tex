...
\SI{6}{\percent} for all trigger chains considered in this search.
...
\item Statistical uncertainty from the finite number of simulated \ttbar events
  in the \hadhad SR:~$\pm \SI{0.9}{\percent}$
...
\ttbarFakes prediction. The uncertainty from the effect of different
\pT~(\ET)~thresholds of \tauhadvis triggers has only a minor impact on the
overall \ttbarFakes background prediction since only a small fraction of \ttbar
events have a \faketauhadvis that is both the leading \tauhadvis candidate and
close to the \pT~threshold of the \tauhadvis-trigger. \Cref{tab:ttbarSF_yields}
summarises the \ttbarFakes prediction in the \hadhad SR and compares it to the
prediction from simulation without application of \faketauhadvis SFs. Generally,
both predictions agree within uncertainties with the exception of the
...
\begin{table}[htbp]
  ...
  \caption{Expected number of \ttbarFakes events in the SR of the \hadhad
    ...
    all systematic uncertainties related to the SF method. }%
