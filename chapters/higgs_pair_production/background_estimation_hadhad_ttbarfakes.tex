...
The expected number of events in bins entering the fit is subject to
...
their ability to alter the the extracted scale factors. In the
...
propagated from dedicated calibration measurements performed in
ATLAS. Other experimental uncertainties originating from trigger
efficiencies of single lepton triggers, efficiency of jet vertex
tagging, reweighting of pile-up conditions in simulation to match the
Run~2 conditions, and the luminosity used to normalise the simulated
...
\begin{figure}[htbp]
  ...
  \caption{Parton shower and hadronisation model uncertainties from
    ...
    threshold on \tauhadvis \pT was relaxed to \SI{20}{\GeV} compared
    to the top control region.}
  \label{fig:ttbarSF_uncertainty_ps}
\end{figure}


\todo{Mention that pT range is altered compared to the measurement}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[scale=0.9]{ttbarSF/correlation_matrix}
