...
detector. Few multi-jet events are selected by STTs due to the high \pT
thresholds on \tauhadvis candidates, preventing a fine binning of the FF
measurement. In this case, FFs are meaasured separately for cases where the
anti-\tauhadvis is leading in \pT and sub-leading in \pT. This accounts for the
\tauid applied at the HLT and the large transverse momentum differences between
the leading and sub-leading \tauhadvis candidate.
...
in~\Cref{fig:fake_factor_OSVR_cutvars} after pre-selection. The contribution of
\Zjets is reduced by rejecting events with di-$\tauhad$ masses close to the \PZ
boson mass.  Multi-jet events are expected to have little real \pTmissAbs, thus
events with a significant \pTmissAbs measurement are rejected to reduce the
\ttbar contribution in this region. The selection increases the multi-jet purity
in the VR to \SI{75}{\percent} with a multi-jet selection efficiency of about
\SI{50}{\percent} with respect to the pre-selection.
...
using simulation. The total background prediction in the multi-jet VR is
...
Good agreement between the background prediction and data is observed in the VR
for \tauhadvis-related observables. Further tests of the assumptions of the FF
method can be performed by a comparison of the FFs measured in the 1 $b$-tag SS
region with FFs obtained from a measurement in the 1 $b$-tag OS multi-jet
...
\Cref{fig:fakefactor_regions}). The change in \btag requirement is not expected
...
Anti-ID is illustrated in~\Cref{fig:mjfakes_2tag_os_antiid}, showing that
