...
\item Fake factor measurement statistical uncertainties.

\item Non-closure uncertainty between fake factors estimated in the 1
  $b$-tag OS and SS regions.

\item Extrapolation uncertainties of fake factors measured in 1
...
The effect of the statistical uncertainty of the fake factor
measurement on the multi-jet estimate in the signal region is
estimated by performing $\pm 1 \sigma$ variations of individual bins
of the measurement. The discriminants used in this analysis are
largely insensitive to variations of individual fake factors and thus
their effect is combined into an overall normalisation uncertainty on
the multi-jet estimate to reduce the number of free parameters of the
background model. The normalisation uncertainty on the multi-jet
prediction in the signal region is \SI{1.4}{\percent}.
...
difference between the central values of OS and SS fake factors as an
uncertainty on the fake factors. The shape effect of this variation on
multi-jet prediction in the signal region is propagated. The
difference between OS and SS fake factors is small, thus the
non-closure uncertainty has only a minor impat on the normalisation of
the multi-jet estimate of \SI{1.0}{\percent}.
...
of freedom originating from extrapolation uncertainties and any shape
...
by performing variations of the subtracted contributions from \ttbar,
\ttbarFakes and other non-multi-jet processes, which are propagated to
the multi-jet prediction.

The normalisation of the subtracted \ttbar component is varied by
$+\SI{16.8}{\percent}$ and $-\SI{18.6}{\percent}$ according to its
modelling uncertainty. The modelling uncertainty is estimated, in
accordance with~\Cref{sec:bkg_hadhad_ttbarfakes}
and~\cite{ATL-PHYS-PUB-2020-023}, by performing variations of: the
hard scatter and PS+NLO matching ($\pm\SI{6.0}{\percent}$), the parton
shower and hadronisation ($\pm\SI{11}{\percent}$), the renormalisation
and factorisation scales ($\substack{+9.9 \\ -9.4}\,\%$ /
$\substack{+2.8 \\ -2.2}\,\%$), the initial- and final-state radiation
($\substack{+0.53 \\ -0.68}\,\%$ / $\substack{+5.4 \\ -10.0}\,\%$).

The subtracted \ttbarFakes component...
\todo[inline]{Continue...}

The normalisation of other non-multi-jet processes, which is a minor
contribution (numbers? \SI{9}{\percent}) mostly originating from
\Vjets and single top production, is varied by \SI{50}{\percent} as a
conservative estimate and propagated to the multi-jet prediction. The
overall uncertainty on the multi-jet prediction in the signal region
is \SI{5.9}{\percent}.

The finite number of events in the 2 $b$-tag OS Anti-ID region and the
simulation-based subtraction of non-multi-jet processes leads to a
statistical uncertainty of \SI{1.9}{\percent} on the multi-jet
normalisation in the signal region.
